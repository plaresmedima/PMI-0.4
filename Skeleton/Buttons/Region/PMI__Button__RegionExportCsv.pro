...
pro PMI__Button__Event__RegionExportCsv, ev
	...
	for i=0L,d[2]-1 do begin
		...
		pmi__write_csv, file_ij, string(float(im))
	endfor
	endfor
...
pro PMI__Button__Control__RegionExportCsv, id, v

	PMI__Info, tlb(id), Region=Region
	widget_control, id, sensitive = obj_valid(Region)
end

function PMI__Button__RegionExportCsv, parent, value=value, separator= separator

	if n_elements(value) eq 0 then value = 'Export region as .csv'

  	return, widget_button(parent $
  	, 	separator	= separator	$
  	, 	value		= value $
	, 	event_pro 	= 'PMI__Button__Event__RegionExportCsv' $
	,	pro_set_value 	= 'PMI__Button__Control__RegionExportCsv' $
	)
