...
pro PMI__Button__Event__RegionImportRaw, ev
...
;GET THE DIRECTORY CONTAINING THE FILES
    ...
    if cnt eq 0 then begin
    	ok = dialog_message(/information,'No data in this directory')
    	goto, return
    endif
...
;LOAD FILES
	...
	for j=0L,d[3]-1 do begin
       ...
       Region -> Write, Stdy->DataPath(), PMI__ReadData(files[k],1), i, j
    endfor
    endfor
...
pro PMI__Button__Control__RegionImportRaw, id, v

	PMI__Info, tlb(id), Series=Series
	widget_control, id, sensitive = obj_valid(Series)
end

function PMI__Button__RegionImportRaw, parent

    return, widget_button(parent,  $
      	value      		= 'RAW',  $
      	event_pro    	= 'PMI__Button__Event__RegionImportRaw', $
    	pro_set_value 	= 'PMI__Button__Control__RegionImportRaw' )
end