...
pro PMI__Button__Event__SeriesExportBmp, ev
	...
	for i=0L,d[2]-1 do begin
		...
		write_bmp, file_ij, reverse(im,1)
	endfor
	endfor
...
pro PMI__Button__Control__SeriesExportBmp, id, v

	PMI__Info, tlb(id), Series=Series
	widget_control, id, sensitive = obj_valid(Series)
end
function PMI__Button__SeriesExportBmp, parent, separator	= separator

  	return, widget_button(parent $
  	, 	separator	= separator	$
  	, 	value		= 'BITMAP' $
	, 	event_pro 	= 'PMI__Button__Event__SeriesExportBmp' $
	,	pro_set_value 	= 'PMI__Button__Control__SeriesExportBmp' $
	)
