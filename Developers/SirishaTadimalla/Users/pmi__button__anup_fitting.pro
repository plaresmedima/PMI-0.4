...
; Default values are set to 4.7T values of v1.0
;
;
;


PRO PMI__Display__anup_fitting::Fit
...
PRO PMI__Display__anup_fitting::Plot
	...
	CASE OnDisplay OF
		...
		'ROI':begin
			...
			xyouts, x0, top-0*dy, 'tissue ROI: ' + RoiName, color=6*16, /normal, charsize=1.5, charthick=1.5
			end
		...
		'AIF':begin
			...
			, 	linestyle=0, thick=2 $
			, 	charsize=1.5, charthick=2.0, xthick=2.0, ythick=2.0
			xyouts, x0, top-1*dy, 'blood ROI: ' + AifName	, color=0, /normal, charsize=1.5, charthick=1.5
			end
...
FUNCTION PMI__Display__anup_fitting::Event, ev
	...
	if cnt eq 1 then begin
		...
		if Uname eq 'Export' then begin
			...
			Path = Stdy->Datapath() + 'ModifiedTofts'
			file_mkdir, Path
		...
		ExportCurves[*,0] = ['Time (s)','Conc (mM)', 'tissue Conc (mM)']
		...
		ExportCurves = strarr(3,1+n_elements(time))
		ExportCurves[*,0] = ['Time (s)','blood (a.u.)', 'tissue (a.u.)']
		ExportCurves[0,1:*] = strcompress(Time,/remove_all)
...
FUNCTION PMI__Display__Event__anup_fitting, ev

	widget_control, ev.handler, get_uvalue=self
	return, Self -> Event(ev)
END

FUNCTION PMI__Display__anup_fitting::Conc, Region
...
;	Self -> GET, SignalModel=SignalModel
	...
	case Region of
		'ROI':begin
			relaxivity = self.Relaxivity
			R10 = self.tissueR1/1000 ; per msec
			end
		'AIF':begin
			relaxivity = self.Relaxivity
			R10 = self.bloodR1/1000 ; per msec
			end
	endcase
	...
	IF self.TR EQ 0 THEN BEGIN
		...
		IF (self.TR EQ 0) OR (self.FA EQ 0) THEN BEGIN
			v = PMI__Form(tlb(self.id), Title='Please enter correct parameters', [$
				ptr_new({Type:'VALUE', Tag:'TR' , Label:'Repetition Time (msec)', Value:5.0E}),$
				ptr_new({Type:'VALUE', Tag:'FA' , Label:'Flip Angle (Degrees)', Value:25E})])
...
FUNCTION PMI__Display__anup_fitting::GetCurve, Region
...
FUNCTION PMI__Display__anup_fitting::GetName, Region
...
PRO PMI__Display__anup_fitting::GET, $
 	CursorPos = CursorPos, $
...
PRO PMI__Display__anup_fitting::SET, $
	...
	bloodR1 = bloodR1, tissueR1 = tissueR1, $
	OnDisplay=OnDisplay
	...
	if n_elements(bloodR1)	ne 0 then self.bloodR1 = bloodR1
	if n_elements(tissueR1) 	ne 0 then self.tissueR1 = tissueR1
...
PRO PMI__Display__anup_fitting::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Display__anup_fitting::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__anup_fitting')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			...
			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['Modified Tofts'])
  			widget_control, id, set_droplist_select = 0, sensitive=0
...
PRO PMI__Display__anup_fitting__Define

	Struct = {PMI__Display__anup_fitting 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__anup_fitting, ev
	...
	v = PMI__Form(ev.top, Title='Perfusion analysis setup', [$
		...
		ptr_new({Type:'DROPLIST',Tag:'aif'	 , Label:'Blood Region', Value:Regions, Select:stdy->sel(1)}), $
		...
		ptr_new({Type:'VALUE'	,Tag:'r1' , Label:' relaxivity (Hz/mM)', Value:5.0E}), $
		ptr_new({Type:'VALUE'	,Tag:'R1_blood' , Label:'Blood relaxation rate (Hz)', Value:0.6944E}), $
		ptr_new({Type:'VALUE'	,Tag:'R1_tissue' , Label:'Tissue relaxation rate (Hz)', Value:1E})])
	IF v.cancel THEN return

	PMI__Control, ev.top, Viewer = 'PMI__Display__anup_fitting', Display=Display

	Display -> Set, /Refresh, $
		Series = Stdy->Obj(0,ind[v.series]), $
		Baseline = v.nbase, $
		Relaxivity = v.r1, $
		bloodR1 = v.R1_blood, tissueR1 = v.R1_tissue, $
		set_droplist_select = [v.roi,v.aif]
end

pro PMI__Button__Control__anup_fitting, id, v
...
function PMI__Button__anup_fitting, parent,value=value, separator=separator

	PMI__Display__anup_fitting__Define

	if n_elements(value) eq 0 then value = 'Anup fitting (ROI)'

	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__anup_fitting',	$
		pro_set_value = 'PMI__Button__Control__anup_fitting', $
	 	separator = separator )
end
