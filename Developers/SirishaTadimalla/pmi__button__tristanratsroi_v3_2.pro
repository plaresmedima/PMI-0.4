...
FUNCTION PMI__Display__tristanratsroi_v3_2::Constants
	...
	Case FieldStrength of
		...
		7.0: begin
			...
			c.R1l = 1.109 ; per sec - liver R1 (Changed from 0.8350 on 06/08/2020)
	...
	fb = 3.61/60. ;liver blood flow (mL/sec/mL) (https://doi.org/10.1021/acs.molpharmaceut.1c00206
	Hct = 0.418 ; (Cremer et al, J Cereb Blood Flow Metab 3, 254-256 (1983).

	c.fp = fb * Hct ;liver plasma flow

	return, c

END

PRO PMI__Display__tristanratsroi_v3_2::Fit
	...
	Fit = mpcurvefit([c.rb,c.rh,c.ve,c.vh,c.fp,ni,ti,Time,aif], $
		Curve, 1+0E*Curve, Pars, $
		function_name='TristanRatModel_v3_2', $
		/quiet,/NODERIVATIVE, PARINFO=parinfo)
	FitError = sqrt(total((Fit-Curve)^2))/sqrt(total((Curve)^2))
	if ni ne n then TristanRatModel_v3_2, [c.rb,c.rh,c.ve,c.vh,c.fp,n,lindgen(n),Time,aif], Pars, Fit
...
PRO PMI__Display__tristanratsroi_v3_2::Plot
...
FUNCTION PMI__Display__tristanratsroi_v3_2::Event, ev
	...
	if cnt eq 1 then begin
		...
		if Uname eq 'Export' then begin
			...
			Path = Stdy->Datapath() + 'TRISTAN Rat Model v3.2 (ROI)'
			file_mkdir, Path
			File = Path + '\' + Roiname
		endif else begin
...
FUNCTION PMI__Display__Event__tristanratsroi_v3_2, ev

	widget_control, ev.handler, get_uvalue=self
	return, Self -> Event(ev)
END

FUNCTION PMI__Display__tristanratsroi_v3_2::DR1, Region
	...
	software_version = self.series->GETVALUE('0018'x,'1020'x)
	IF software_version EQ 'ParaVision Acquisition 5.1\ParaVision 5.1 ' THEN FA=30.0
...
FUNCTION PMI__Display__tristanratsroi_v3_2::GetCurve, Region, INDICES=TimeIndices
...
FUNCTION PMI__Display__tristanratsroi_v3_2::GetName, Region
...
PRO PMI__Display__tristanratsroi_v3_2::GET, $
 	CursorPos = CursorPos, $
...
PRO PMI__Display__tristanratsroi_v3_2::SET, $
	...
	OnDisplay=OnDisplay, FieldStrength = FieldStrength
	...
	if n_elements(FieldStrength) 	ne 0 then Self.FieldStrength = FieldStrength
...
PRO PMI__Display__tristanratsroi_v3_2::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Indices, self.Parameters
	loadct, 0
END

FUNCTION PMI__Display__tristanratsroi_v3_2::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__tristanratsroi_v3_2')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			...
			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['TRISTAN Rat Model v3.2'])
  			widget_control, id, set_droplist_select = 0, sensitive=0
...
PRO PMI__Display__tristanratsroi_v3_2__Define

	Struct = {PMI__Display__tristanratsroi_v3_2 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__tristanratsroi_v3_2, ev
	...
	FieldStrength = FLOOR(series->GETVALUE('0018'x,'0087'x))
	IF FieldStrength EQ 0 THEN BEGIN
		in = PMI__Form(ev.top, Title='Please enter the correct field strength.', [$
			ptr_new({Type:'VALUE',Tag:'field', Label:'Field strength (Tesla)', Value:7D})])
		IF in.cancel THEN return
		FieldStrength = FLOOR(in.field)
	ENDIF
	IF NOT ((FieldStrength EQ 4.0) OR (FieldStrength EQ 7.0)) THEN BEGIN
		msg = 'Sorry, only field strengths of 4T and 7T are currently supported.'
		OK = dialog_message(msg,/information)
		return
	ENDIF

	PMI__Control, ev.top, Viewer = 'PMI__Display__tristanratsroi_v3_2', Display=Display
...
pro PMI__Button__Control__tristanratsroi_v3_2, id, v
...
function PMI__Button__tristanratsroi_v3_2, parent,value=value, separator=separator

	PMI__Display__tristanratsroi_v3_2__Define

	if n_elements(value) eq 0 then value = 'TRISTAN RAT models (ROI)'

	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__tristanratsroi_v3_2',	$
		pro_set_value = 'PMI__Button__Control__tristanratsroi_v3_2', $
	 	separator = separator )
end
