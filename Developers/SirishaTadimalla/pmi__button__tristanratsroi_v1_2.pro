...
PRO PMI__Display__tristanratsroi_v1_2::Fit
...
PRO PMI__Display__tristanratsroi_v1_2::Plot
...
FUNCTION PMI__Display__tristanratsroi_v1_2::Event, ev
...
FUNCTION PMI__Display__Event__tristanratsroi_v1_2, ev

	widget_control, ev.handler, get_uvalue=self
	return, Self -> Event(ev)
END

FUNCTION PMI__Display__tristanratsroi_v1_2::Conc, Region
...
;	Self -> GET, SignalModel=SignalModel
	...
	case Region of
		'ROI':begin
			...
			R10 = self.LiverR1/1000 ; per msec
			end
		'AIF':begin
			relaxivity = self.SpleenRelaxivity
			R10 = self.SpleenR1/1000 ; per msec
			end
	endcase
...
FUNCTION PMI__Display__tristanratsroi_v1_2::GetCurve, Region
...
FUNCTION PMI__Display__tristanratsroi_v1_2::GetName, Region
...
PRO PMI__Display__tristanratsroi_v1_2::GET, $
 	CursorPos = CursorPos, $
...
PRO PMI__Display__tristanratsroi_v1_2::SET, $
	...
	SpleenR1 = SpleenR1, LiverR1 = LiverR1, $
	OnDisplay=OnDisplay
	...
	if n_elements(SpleenR1)	ne 0 then self.SpleenR1 = SpleenR1
	if n_elements(LiverR1) 	ne 0 then self.LiverR1 = LiverR1
...
PRO PMI__Display__tristanratsroi_v1_2::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Display__tristanratsroi_v1_2::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__tristanratsroi_v1_2')
...
PRO PMI__Display__tristanratsroi_v1_2__Define

	SingleInletTristanRatModel

	Struct = {PMI__Display__tristanratsroi_v1_2 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__tristanratsroi_v1_2, ev
	...
	v = PMI__Form(ev.top, Title='Perfusion analysis setup', [$
		...
		ptr_new({Type:'VALUE'	,Tag:'R1_spleen' , Label:'Spleen relaxation rate (Hz)', Value:0.6313E}), $
		ptr_new({Type:'VALUE'	,Tag:'R1_liver' , Label:'Liver relaxation rate (Hz)', Value:0.8346E})])
	IF v.cancel THEN return

	PMI__Control, ev.top, Viewer = 'PMI__Display__tristanratsroi_v1_2', Display=Display

	Display -> Set, /Refresh, $
		Series = Stdy->Obj(0,ind[v.series]), $
		Baseline = v.nbase, $
		SpleenRelaxivity = v.rspleen, LiverRelaxivity = v.rliver, $
		SpleenR1 = v.R1_spleen, LiverR1 = v.R1_liver, $
		set_droplist_select = [v.roi,v.aif]
end

pro PMI__Button__Control__tristanratsroi_v1_2, id, v
...
function PMI__Button__tristanratsroi_v1_2, parent,value=value, separator=separator

	PMI__Display__tristanratsroi_v1_2__Define

	if n_elements(value) eq 0 then value = 'TRISTAN RAT models (ROI)'

	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__tristanratsroi_v1_2',	$
		pro_set_value = 'PMI__Button__Control__tristanratsroi_v1_2', $
	 	separator = separator )
end
