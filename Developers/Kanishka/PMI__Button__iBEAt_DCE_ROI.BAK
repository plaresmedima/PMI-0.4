...
PRO PMI__Display__iBEAt_DCE_ROI::Fit
	...
	CASE Model OF
		...
		'Maximum slope':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Flow'		,Units:'ml/100ml/min'	,Value:6000D*flow/(1D -self.Hematocrit),Nr: 0}	]
			end
		...
		'Model-free':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Flow'		,Units:'ml/100ml/min'	,Value:6000D*max(IRF)/(1D -self.Hematocrit)			,Nr: 0} $
		...
		'Patlak':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Volume'		,Units:'ml/100ml'		,Value:100D*P[0]/(1D -self.Hematocrit)	,Nr: 2} $
				,{Name:'Tubular Flow'		,Units:'ml/100ml/min'	,Value:6000D*P[1]	,Nr: 4} $
				,{Name:'GFR'		,Units:'ml/min'			,Value:Vol*60D*P[1]	,Nr: 8} $
				,{Name:'Volume'		,Units:'ml'				,Value:Vol	,Nr: 10} ]
			IF Delay NE 0 THEN $
		...
		'Modified Tofts':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Volume'		,Units:'ml/100ml'		,Value:100D*P[0]*(1-P[1])/(1D -self.Hematocrit)	,Nr: 2} $
				,{Name:'Tubular MTT'		,Units:'min'			,Value:1D*P[0]*P[1]/P[2]/60D		,Nr: 5} $
				,{Name:'Tubular Flow'		,Units:'ml/100ml/min'	,Value:6000D*P[2]	,Nr: 4} $
				,{Name:'GFR'		,Units:'ml/min'			,Value:Vol*60D*P[2]	,Nr: 8} $
				,{Name:'Volume'		,Units:'ml'				,Value:Vol	,Nr: 10}]
			IF Delay NE 0 THEN $
				Parameters = [Parameters,{Name:'Arterial Delay', Units:'sec', Value:1D*Pd, Nr:12} ]
			end
		...
		'2C Uptake':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Flow'			,Units:'ml/100ml/min'	,Value:6000D*P[1]/(1D -self.Hematocrit)		,Nr: 0} $
				,{Name:'Tissue Blood MTT'				,Units:'sec'			,Value:1D*P[0]/P[1] 		,Nr: 1} $
				,{Name:'Tissue Blood Volume'			,Units:'ml/100ml'		,Value:100D*P[0]/(1-self.Hematocrit)		,Nr: 2} $
				,{Name:'Filtration Fraction'	,Units:'%' 				,Value:100D*P[2]	,Nr: 7} $
				,{Name:'Tubular Flow'			,Units:'ml/100ml/min'	,Value:6000D*P[1]*P[2]		,Nr: 4} $
				,{Name:'GFR'			,Units:'ml/min'			,Value:Vol*60D*P[1]*P[2]	,Nr: 8} $
				,{Name:'Volume'		,Units:'ml'				,Value:Vol	,Nr: 10}]
			IF Delay NE 0 THEN $
				Parameters = [Parameters,{Name:'Arterial Delay', Units:'sec', Value:1D*Pd, Nr:12} ]
			end
		...
		'2C Filtration':begin
			...
			Parameters = $
				[{Name:'Tissue Blood Flow'			,Units:'ml/100ml/min'	,Value:6000D*P[1]/(1D -self.Hematocrit)			,Nr: 0} $
				,{Name:'Tissue Blood MTT'			,Units:'sec'			,Value:1D*P[0]*(1-P[2])/P[1]					,Nr: 1} $
				,{Name:'Tissue Blood Volume'		,Units:'ml/100ml'		,Value:100D*P[0]*(1-P[2])/(1D -self.Hematocrit)	,Nr: 2} $
				,{Name:'Tubular MTT'		,Units:'min'			,Value:1D*P[0]*P[2]/(P[1]*P[3])/60D	,Nr: 5} $
				,{Name:'Filtration Fraction',Units:'%' 				,Value:100D*P[3]				,Nr: 7} $
				,{Name:'Tubular Flow'		,Units:'ml/100ml/min'	,Value:6000D*P[1]*P[3]			,Nr: 4} $
				,{Name:'GFR'		,Units:'ml/min'			,Value:Vol*60D*P[1]*P[3]		,Nr: 8} $
				,{Name:'Blood Flow',Units:'ml/min'			,Value:Vol*60D*P[1]/(1D -self.Hematocrit)		,Nr: 9} $
				,{Name:'Volume'		,Units:'ml'				,Value:Vol						,Nr: 10} $
...
PRO PMI__Display__iBEAt_DCE_ROI::Plot
...
FUNCTION PMI__Display__iBEAt_DCE_ROI::Event, ev
...
FUNCTION PMI__Display__Event__iBEAt_DCE_ROI, ev
...
FUNCTION PMI__Display__iBEAt_DCE_ROI::Conc, Region
	...
	If SignalModel eq 3 then return, Concentration_SPGRESS(Signal, S0, 1/R10, FA, TR, relaxivity)

	TI = self.series->GETVALUE('0018'x,'0082'x)
	Nph = self.series->GETVALUE('0018'x,'0089'x)
	DelayTime = TI - TR*Nph/2

	If SignalModel eq 4 then return, Concentration_SR_spGRE(Signal, S0, 1/R10, relaxivity, DelayTime, FA, TR, Nph/2)
END



FUNCTION PMI__Display__iBEAt_DCE_ROI::GetCurve, Region
...
FUNCTION PMI__Display__iBEAt_DCE_ROI::GetName, Region
...
PRO PMI__Display__iBEAt_DCE_ROI::GET, $
 	CursorPos = CursorPos, $
...
PRO PMI__Display__iBEAt_DCE_ROI::SET, $
...
PRO PMI__Display__iBEAt_DCE_ROI::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Display__iBEAt_DCE_ROI::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__iBEAt_DCE_ROI')
...
PRO PMI__Display__iBEAt_DCE_ROI__Define
	...
	Struct = {PMI__Display__iBEAt_DCE_ROI 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__iBEAt_DCE_ROI, ev
	...
	v = PMI__Form(ev.top, Title='Perfusion analysis setup', [$
		...
		ptr_new({Type:'VALUE'	,Tag:'nbase' , Label:'Length of baseline (# of dynamics)', Value:15L}),$
	...
	PMI__Control, ev.top, Viewer = 'PMI__Display__iBEAt_DCE_ROI', Display=Display
...
pro PMI__Button__Control__iBEAt_DCE_ROI, id, v
...
function PMI__Button__iBEAt_DCE_ROI, parent,value=value, separator=separator

	PMI__Display__iBEAt_DCE_ROI__Define

	if n_elements(value) eq 0 then value = 'Renal filtration models (ROI)'

	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__iBEAt_DCE_ROI',	$
		pro_set_value = 'PMI__Button__Control__iBEAt_DCE_ROI', $
	 	separator = separator )
end
