...
PRO PMI__Display__iBEAt_T2star_ROI::Fit
	...
	CASE Model OF
		...
		'T2starMap':begin
            ...
            P = [0.01*max(Signal),10.0,0.99*max(Signal),40.0] ;P = [Sfat, T2*fat, Swater, T2*water]

	    	Fit = MoCoModelFit(Curve, 'T2star' , Time, PARAMETERS=P)

			Parameters = $
				[{Name:'S0'		            ,Units:'a.u.'		,Value:P[0,0]+P[2,0]	,Nr: 0} $
				,{Name:'T2star fat'		    ,Units:'ms'	,Value:P[1,0]	,Nr: 1} $
				,{Name:'T2star water'		,Units:'ms'	,Value:P[3,0]	,Nr: 2} $
				,{Name:'fat fraction'		,Units:'%'	,Value:100*P[0,0]/(P[0,0]+P[2,0]),Nr: 3} ]
...
PRO PMI__Display__iBEAt_T2star_ROI::Plot
	...
	CASE OnDisplay OF
		...
		'ROI':begin
			...
			, 	xtitle = 'Echo Time (sec)', ytitle=Units $
		...
		'FIT':BEGIN
			...
			xyouts, x0, top-0*dy, 'Region Of Interest: ' + RoiName		, color=6*16, /normal, charsize=1.5, charthick=1.5
			xyouts, x0, top-3*dy, 'T2star Tissue Model: ' + Model				, color=12*16, /normal, charsize=1.5, charthick=1.5
...
FUNCTION PMI__Display__iBEAt_T2star_ROI::Event, ev
	...
	if cnt eq 1 then begin
		...
		if Uname eq 'Export' then begin
			...
			Path = Stdy->Datapath() + 'T2star Kidney Models (ROI)'
			file_mkdir, Path
...
FUNCTION PMI__Display__Event__iBEAt_T2star_ROI, ev
...
FUNCTION PMI__Display__iBEAt_T2star_ROI::Region_Of_Interest, Region ;
...
FUNCTION PMI__Display__iBEAt_T2star_ROI::GetCurve, Region
...
FUNCTION PMI__Display__iBEAt_T2star_ROI::GetName, Region
...
PRO PMI__Display__iBEAt_T2star_ROI::GET, $
 	CursorPos = CursorPos, $
	Model=Model, $
...
PRO PMI__Display__iBEAt_T2star_ROI::SET, $
...
PRO PMI__Display__iBEAt_T2star_ROI::Cleanup
...
FUNCTION PMI__Display__iBEAt_T2star_ROI::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__iBEAt_T2star_ROI')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_droplist(Base,/dynamic_resize, uname='SIG',value = ['T2star Signal (a.u.)'])
  			widget_control, id, set_droplist_select = 4

		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['T2starMap'])
...
PRO PMI__Display__iBEAt_T2star_ROI__Define

	MoCoModel_T2star__define ; modelfit

	Struct = {PMI__Display__iBEAt_T2star_ROI 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__iBEAt_T2star_ROI, ev
	...
	v = PMI__Form(ev.top, Title='T2star map analysis setup', [$
		ptr_new({Type:'DROPLIST',Tag:'series', Label:'T2star series', Value:Series, Select:sel}), $
	...
	PMI__Control, ev.top, Viewer = 'PMI__Display__iBEAt_T2star_ROI', Display=Display
...
pro PMI__Button__Control__iBEAt_T2star_ROI, id, v
...
function PMI__Button__iBEAt_T2star_ROI, parent,value=value, separator=separator
	...
	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__iBEAt_T2star_ROI',	$
		pro_set_value = 'PMI__Button__Control__iBEAt_T2star_ROI', $
	 	separator = separator )
end
