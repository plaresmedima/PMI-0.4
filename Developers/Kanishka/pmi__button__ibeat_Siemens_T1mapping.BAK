FUNCTION PMI__Button__Input__ibeat_Siemens_T1mapping, top, series
...
pro PMI__Button__Event__ibeat_Siemens_T1mapping, ev

	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__ibeat_Siemens_T1mapping(ev.top,series) THEN goto, exit
...
pro PMI__Button__Control__ibeat_Siemens_T1mapping, id, v
...
function PMI__Button__ibeat_Siemens_T1mapping, parent, value=value,separator=separator

	PMI__FitT1SaturationRecovery

    if n_elements(value) eq 0 then value = 'T1 mapping'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__ibeat_Siemens_T1mapping' $
    ,	pro_set_value = 'PMI__Button__Control__ibeat_Siemens_T1mapping' $
    ,  	separator = separator )
