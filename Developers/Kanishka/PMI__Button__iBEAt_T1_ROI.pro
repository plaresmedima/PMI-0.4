...
PRO PMI__Display__iBEAt_T1_ROI::Fit
    ...
    Time = {TI:TI[1,*]} ; to change TI for corresponding slice number

    Signal= *Self.Curve[0]

    Curve = reform(Signal,[n_elements(TI[1,*]),1])
...
PRO PMI__Display__iBEAt_T1_ROI::Plot
	...
	CASE OnDisplay OF
		...
		'ROI':begin
			...
			, 	xtitle = 'TI Time (msec)', ytitle=Units $
		...
		'FIT':BEGIN
			...
			xyouts, x0, top-0*dy, 'Region Of Interest: ' + RoiName		, color=6*16, /normal, charsize=1.5, charthick=1.5
			xyouts, x0, top-3*dy, 'T1 Tissue Model: ' + Model				, color=12*16, /normal, charsize=1.5, charthick=1.5
...
FUNCTION PMI__Display__iBEAt_T1_ROI::Event, ev ; add event for roi name
	...
	if cnt eq 1 then begin
		...
		if Uname eq 'Export' then begin
			...
			Path = Stdy->Datapath() + 'T1 Kidney Models (ROI)'
			file_mkdir, Path
...
FUNCTION PMI__Display__Event__iBEAt_T1_ROI, ev
...
FUNCTION PMI__Display__iBEAt_T1_ROI::GetCurve, Region
...
FUNCTION PMI__Display__iBEAt_T1_ROI::GetName, Region
...
PRO PMI__Display__iBEAt_T1_ROI::GET, $
 	CursorPos = CursorPos, $
	Model=Model, $
...
PRO PMI__Display__iBEAt_T1_ROI::SET, $
...
PRO PMI__Display__iBEAt_T1_ROI::Cleanup
...
FUNCTION PMI__Display__iBEAt_T1_ROI::Init, parent, CursorPos, xsize=xsize, ysize=ysize ; DEFINE DISPLAY: CHANGE ROI
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__iBEAt_T1_ROI')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_droplist(Base,/dynamic_resize, uname='SIG',value = ['T1 MOLLI Signal (a.u.)']) ; name of the signal curve units
  			widget_control, id, set_droplist_select = 4

		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['T1MapMOLLI']) ; name of the fitting
...
PRO PMI__Display__iBEAt_T1_ROI__Define

	MoCoModel_T1MapMOLLI__define ; modelfit

	Struct = {PMI__Display__iBEAt_T1_ROI 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__iBEAt_T1_ROI, ev
	...
	v = PMI__Form(ev.top, Title='T1 map analysis setup', [$
		ptr_new({Type:'DROPLIST',Tag:'series', Label:'T1 series', Value:Series, Select:sel}), $
	...
	PMI__Control, ev.top, Viewer = 'PMI__Display__iBEAt_T1_ROI', Display=Display
...
pro PMI__Button__Control__iBEAt_T1_ROI, id, v
...
function PMI__Button__iBEAt_T1_ROI, parent,value=value, separator=separator
	...
	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__iBEAt_T1_ROI',	$
		pro_set_value = 'PMI__Button__Control__iBEAt_T1_ROI', $
	 	separator = separator )
end
