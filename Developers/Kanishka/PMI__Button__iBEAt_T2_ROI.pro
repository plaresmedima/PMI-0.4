...
PRO PMI__Display__iBEAt_T2_ROI::Fit
...
PRO PMI__Display__iBEAt_T2_ROI::Plot
	...
	CASE OnDisplay OF
		...
		'FIT':BEGIN
			...
			xyouts, x0, top-0*dy, 'Region Of Interest: ' + RoiName		, color=6*16, /normal, charsize=1.5, charthick=1.5
			xyouts, x0, top-3*dy, 'T2 Tissue Model: ' + Model				, color=12*16, /normal, charsize=1.5, charthick=1.5
...
FUNCTION PMI__Display__iBEAt_T2_ROI::Event, ev
	...
	if cnt eq 1 then begin
		...
		if Uname eq 'Export' then begin
			...
			Path = Stdy->Datapath() + 'T2 Kidney Models (ROI)'
			file_mkdir, Path
...
FUNCTION PMI__Display__Event__iBEAt_T2_ROI, ev
...
FUNCTION PMI__Display__iBEAt_T2_ROI::Region_Of_Interest, Region ;
...
FUNCTION PMI__Display__iBEAt_T2_ROI::GetCurve, Region
...
FUNCTION PMI__Display__iBEAt_T2_ROI::GetName, Region
...
PRO PMI__Display__iBEAt_T2_ROI::GET, $
 	CursorPos = CursorPos, $
	Model=Model, $
...
PRO PMI__Display__iBEAt_T2_ROI::SET, $
...
PRO PMI__Display__iBEAt_T2_ROI::Cleanup
...
FUNCTION PMI__Display__iBEAt_T2_ROI::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Display__Event__iBEAt_T2_ROI')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_droplist(Base,/dynamic_resize, uname='SIG',value = ['T2 Signal (a.u.)'])
  			widget_control, id, set_droplist_select = 4

		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['T2Map'])
...
PRO PMI__Display__iBEAt_T2_ROI__Define

	MoCoModel_T2Map__define ; modelfit

	Struct = {PMI__Display__iBEAt_T2_ROI 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__iBEAt_T2_ROI, ev
	...
	v = PMI__Form(ev.top, Title='T2 map analysis setup', [$
	...
	PMI__Control, ev.top, Viewer = 'PMI__Display__iBEAt_T2_ROI', Display=Display
...
pro PMI__Button__Control__iBEAt_T2_ROI, id, v
...
function PMI__Button__iBEAt_T2_ROI, parent,value=value, separator=separator
	...
	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__iBEAt_T2_ROI',	$
		pro_set_value = 'PMI__Button__Control__iBEAt_T2_ROI', $
	 	separator = separator )
end
