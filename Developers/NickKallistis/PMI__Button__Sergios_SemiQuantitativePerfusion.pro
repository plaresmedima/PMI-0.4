...
pro PMI__Button__Event__Sergios_SemiQuantitativePerfusion, ev
	...
	, ptr_new({Type:'DROPLIST',Tag:'tmax',Label: 'Limit to 100 sec?', Value:['Yes','No'], Select:0}) $
    ...
    max_t = d[3]-1
    if v.tmax eq 0 then begin
      near = Min(Abs(time - 100), max_t)
    endif
	...
	for i=0L,d[2]-1 do begin
		...
		MAX->Trim, max([MAX->Trim(1),200]), 1
		AUC->Trim, max([AUC->Trim(1),3*median(AUCim)]), 1
...
pro PMI__Button__Control__Sergios_SemiQuantitativePerfusion, id, v
...
function PMI__Button__Sergios_SemiQuantitativePerfusion, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Semi-quantitative parameters (Pixel)'

    id = widget_button(parent $
    , 	value 		= value $
	, 	event_pro	= 'PMI__Button__Event__Sergios_SemiQuantitativePerfusion' $
	,	pro_set_value 	= 'PMI__Button__Control__Sergios_SemiQuantitativePerfusion' $
	, 	separator 	= separator	)
