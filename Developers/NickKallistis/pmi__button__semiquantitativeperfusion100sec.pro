...
pro PMI__Button__Event__SemiQuantitativePerfusion100sec, ev
	...
	, ptr_new({Type:'DROPLIST',Tag:'units',Label: 'Signal model', Value:['Linear (a.u.)','Linear (%)','DSC-MRI'], Select:1}) $
	...
	MAX = Stdy->New('SERIES',Default=Series,Name=Series->Name()+'[Maximum 100 sec ('+U+')]') 					& MAX->t, Series->t(0)
	AUC = Stdy->New('SERIES',Default=Series,Name=Series->Name()+'[Area under the Curve ('+U+'*100 sec)]')	& AUC->t, Series->t(0)
	MTT = Stdy->New('SERIES',Default=Series,Name=Series->Name()+'[Area:Maximum 100 (sec)]')					& MTT->t, Series->t(0)
	...
	time = Series->t()
	near = Min(Abs(time - 100), index)

	time = float(time[0:index]-time[0])
	d = Series->d()
...
pro PMI__Button__Control__SemiQuantitativePerfusion100sec, id, v
...
function PMI__Button__SemiQuantitativePerfusion100sec, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Semi-quantitative parameters (Pixel)'

    id = widget_button(parent $
    , 	value 		= value $
	, 	event_pro	= 'PMI__Button__Event__SemiQuantitativePerfusion100sec' $
	,	pro_set_value 	= 'PMI__Button__Control__SemiQuantitativePerfusion100sec' $
	, 	separator 	= separator	)
