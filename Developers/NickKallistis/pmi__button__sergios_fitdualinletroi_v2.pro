...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::Plot
	...
	CASE OnDisplay OF
		...
		'ROI':begin
			Self -> GET, Timet=Timet, RoiCurve=Y, RoiName=RoiName
			Self -> SET, /Erase
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
  			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['Compartment','Patlak','Modified Tofts', 'Uptake','Exchange','Exchange-Uptake'])
