...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::Fit

	Self->GET, Model=Model, AIF_Delay=AIF_Delay, VIF_Delay=VIF_Delay, AIF_Use=AIF_Use, VIF_Use=VIF_Use, Timei=Time, Timet=Timet, AifCurve=Aif, VifCurve=Vif, RoiCurve=Curve, OnDisplay=OnDisplay
	Self->SET, Message='Fitting...', Sensitive=0

    curve = interpol(curve,timet,time)
...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::Plot
	...
	CASE OnDisplay OF
		...
		'ROI':begin
			Self -> GET, Timei=timei, Timet=Timet, RoiCurve=Y, RoiName=RoiName
			Self -> SET, /Erase
		...
		'AIF':begin
			Self -> GET, Timei=Timei, AifCurve=Y, AifName=AifName
			Self -> SET, /Erase
 			plot, timei, Y, position=[0.1,0.2,0.5,0.9]  $
			, 	/xstyle, /ystyle $
			, 	background=255, color=0 $
			, 	xtitle = 'Time (sec)', ytitle=self.Units $
			, 	linestyle=0, thick=2 $
		...
		'VIF':begin
			Self -> GET, Timei=Timei, VifCurve=Y, VifName=VifName
			Self -> SET, /Erase
 			plot, timei, Y, position=[0.1,0.2,0.5,0.9]  $
			, 	/xstyle, /ystyle $
			, 	background=255, color=0 $
...
FUNCTION PMI__Button__Display__Sergios_FitDualInletRoi_v2::Event, ev
	...
	if cnt eq 1 then begin
		Self->GET, Timet=Timet, RoiCurve=RoiCurve, Fit=Fit, Model=Model, Roiname=Roiname
		...
		PMI__WritePlot, File + '__Curve.txt', Timet, RoiCurve
		PMI__WritePlot, File + '__Fit.txt', timet, Fit
...
FUNCTION PMI__Button__Display__Event__Sergios_FitDualInletRoi_v2, ev
...
FUNCTION PMI__Button__Display__Sergios_FitDualInletRoi_v2::GetName, List
...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::GET $
, 	CursorPos = CursorPos $
,	Model=Model, AIF_Delay=AIF_Delay, VIF_Delay=VIF_Delay $
, 	AIF_Use=AIF_Use, VIF_Use=VIF_Use $
,	Timei=Timei, Timet=Timet, Fit=Fit $
...
, 	OnDisplay=OnDisplay
	...
	if arg_present(Timei) then begin
		t = Self.Seriesi -> c(1)
		Timei = t-t[0]
	endif
	if arg_present(Timet) then begin
		t = Self.Seriest -> c(1)
		Timet = t-t[0]
	endif
...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::SET, $
	...
	Seriesi=Seriesi, Seriest=Seriest, Units=Units, Baseline=Baseline, Hematocrit=Hematocrit, $
	...
	if n_elements(Seriesi) 	ne 0 then Self.Seriesi = Seriesi
	if n_elements(Seriest) 	ne 0 then Self.Seriest = Seriest
...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Button__Display__Sergios_FitDualInletRoi_v2::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Button__Display__Event__Sergios_FitDualInletRoi_v2')
...
PRO PMI__Button__Display__Sergios_FitDualInletRoi_v2__Define
	...
	Struct = {PMI__Button__Display__Sergios_FitDualInletRoi_v2 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__Sergios_FitDualInletRoi_v2, ev

	PMI__Info, ev.top, Stdy=Stdy

    Series = Stdy->Names(0,ns,DefDim=3,ind=ind,sel=sel)
    Regions = Stdy->Names(1,nr)
    Units = ['Linear (a.u.)','Linear (%)','Linear (mM)','Non-linear (mM)']
    ...
    Seriesi = Stdy->Obj(0,ind[v.seriesi])
	...
	IF v.units EQ 2 THEN BEGIN
		...
		p = PMI__Form(ev.top, Title='Check parameters', [$
			ptr_new({Type:'VALUE', Tag:'Relaxivity' , Label:'Relaxivity ', Value:3.6E}),$
			ptr_new({Type:'VALUE', Tag:'T1t' , Label:'Tissue T1 (msec)', Value:809E}), $
			ptr_new({Type:'VALUE', Tag:'T1a' , Label:'Arterial Blood T1 (msec)', Value:T1a}), $
			ptr_new({Type:'VALUE', Tag:'T1v' , Label:'Venous Blood T1 (msec)', Value:T1v}) $
			])
		IF p.cancel THEN return
	ENDIF

	IF v.units EQ 3 THEN BEGIN
	...
	PMI__Control, ev.top, Viewer = 'PMI__Button__Display__Sergios_FitDualInletRoi_v2', Display=Display
	...
	if v.units EQ 2 THEN Display -> Set, Relaxivity=p.Relaxivity, T1t=p.T1t, T1a=p.T1a, T1v=p.T1v
	if v.units EQ 3 THEN Display -> Set, Relaxivity=p.Relaxivity, T1t=p.T1t, T1a=p.T1a, T1v=p.T1v, TR=p.TR, FA=p.FA

	Display -> Set, /Refresh
end

pro PMI__Button__Control__Sergios_FitDualInletRoi_v2, id, v
...
function PMI__Button__Sergios_FitDualInletRoi_v2, parent,value=value, separator=separator

	PMI__Button__Display__Sergios_FitDualInletRoi_v2__Define

	if n_elements(value) eq 0 then value = 'Fit dual-inlet models (ROI)'

	id = widget_button(parent $
	,	value = value	$
	,	event_pro = 'PMI__Button__Event__Sergios_FitDualInletRoi_v2'	$
	,	pro_set_value = 'PMI__Button__Control__Sergios_FitDualInletRoi_v2' $
	, 	separator = separator)
