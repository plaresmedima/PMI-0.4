FUNCTION PMI__Button__Input__DualInletUptakePixel, ev $
	,	Stdy		= Stdy $
	,	status 		= status $
	,	time 		= time $
	,	pixelcurve 	= pcurve $
    ...
    Vif = LMU__Enhancement(Vif,in.nb,relative=in.rel)/(1-in.hct)

	np = n_elements(pcurve[*,0])
	for i=0L,np-1 do begin
		PMI__Message, id, 'Calculating concentrations ', i/(np-1E)
		pcurve[i,*] = LMU__Enhancement(pcurve[i,*],in.nb,relative=in.rel)
	endfor
...
pro PMI__Button__Event__DualInletUptakePixel, ev
	...
	IF NOT PMI__Button__Input__DualInletUptakePixel(ev $
	,	Stdy		= Stdy $
	,	status 		= status $
	,	time 		= time $
	,	pixelcurve 	= p $
