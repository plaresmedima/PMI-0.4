FUNCTION PMI__Button__Input__Sergios_DualInletUptakePixel_delayAIF, ev $
	,	Stdy		= Stdy $
	,	status 		= status $
	,	time 		= time $
	,	pixelcurve 	= pcurve $
...
pro PMI__Button__Event__Sergios_DualInletUptakePixel_delayAIF, ev

	IF NOT PMI__Button__Input__Sergios_DualInletUptakePixel_delayAIF(ev $
	,	Stdy		= Stdy $
	,	status 		= status $
	,	time 		= time $
	,	pixelcurve 	= p $
   ...
   endwhile
	...
	for i=0L,n-1 do begin
		...
		DELAY_VALUES=[0,20,1]
		DELAY_WHICH = 0
		Pars = [0.2*50/6000D,0.8*50/6000.,0.2,0.2] ;P = [FA,FV,VE,EI]
		Fit = FitDualInlet('Uptake', time, aif, vif, reform(p[i,*]), Pars, LIMITED_ABOVE=[1,1,1,1], LIMITS_ABOVE=[1000/6000D,1000/6000D,1,1D], $
		...
		MT[ind[i]] 	= 1D*Pars[2]*(1-Pars[3])/(Pars[0]+Pars[1])
		VL[ind[i]]	= 100D*Pars[2]
		UF[ind[i]]	= 100*Pars[3]
		UP[ind[i]]	= 600D*(Pars[0]+Pars[1])*Pars[3]/(1-Pars[3])
		DL[ind[i]] = 1D*Pd
	...
	Suf = Stdy->New('SERIES', Domain=Domain, Data=remove_inf(UF),	Name='2-inlet 2-compartment model - Uptake Rate (/100/min)')
	Sdl = Stdy->New('SERIES', Domain=Domain, Data=remove_inf(DL),	Name='2-inlet 2-compartment model - Arterial Delay time  (sec)')
...
pro PMI__Button__Control__Sergios_DualInletUptakePixel_delayAIF, id, v
...
function PMI__Button__Sergios_DualInletUptakePixel_delayAIF, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Fit dual input 2-compartment model (Pixel)'

	id = widget_button(parent $
	,	value 		= value	$
	,	event_pro 	= 'PMI__Button__Event__Sergios_DualInletUptakePixel_delayAIF'	$
	,	pro_set_value 	= 'PMI__Button__Control__Sergios_DualInletUptakePixel_delayAIF' $
	, 	separator 	= separator	)
