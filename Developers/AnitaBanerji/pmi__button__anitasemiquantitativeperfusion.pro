...
pro PMI__Button__Event__AnitaSemiQuantitativePerfusion, ev
	...
	TTP = Stdy->New('SERIES',Default=Series,Name=Series->Name()+'[Time To Peak (sec)]')					& TTP->t, Series->t(0)
...
pro PMI__Button__Control__AnitaSemiQuantitativePerfusion, id, v
...
function PMI__Button__AnitaSemiQuantitativePerfusion, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Semi-quantitative parameters (Pixel)'

    id = widget_button(parent $
    , 	value 		= value $
	, 	event_pro	= 'PMI__Button__Event__AnitaSemiQuantitativePerfusion' $
	,	pro_set_value 	= 'PMI__Button__Control__AnitaSemiQuantitativePerfusion' $
	, 	separator 	= separator	)
