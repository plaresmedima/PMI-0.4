...
PRO PMI__Button__RoiDCEurLAB__Display::Fit
...
PRO PMI__Button__RoiDCEurLAB__Display::Plot
...
FUNCTION PMI__Button__RoiDCEurLAB__Display::Event, ev
...
FUNCTION PMI__Button__RoiDCEurLAB__Display__Event, ev
...
FUNCTION PMI__Button__RoiDCEurLAB__Display::GetCurve, List
	...
	case self.units of
	'Linear (a.u.)': return, LMU__Enhancement(Y,Self.Baseline,relative=0)
	'Linear (%)': return, LMU__Enhancement(Y,Self.Baseline,relative=1)
	'DSC-MRI': return, LMU__Enhancement(Y,Self.Baseline,relative=2)
	'DCE@urLAB': begin
		   TR = 5.05 ;ms
		   FA = 17.0 ;degrees
		   T10 = 1000.0 ;ms
		   ...
		   RE = (Y-S0)/S0
	;	   X = RE/sFA + (1-E10)/(1-E10*cFA)
	;	   R1 = remove_inf(-alog((1-X)/(1-X*cFA))/TR)
		   X = RE*(E10-1)
		   R1 = remove_inf(-alog((X+E10*(1-cFA))/(1+(X-1)*cFA))/TR)
		   return, R1 - total(R1[0:Self.Baseline-1])/Self.Baseline
		   end
	endcase

END

FUNCTION PMI__Button__RoiDCEurLAB__Display::GetName, List
...
PRO PMI__Button__RoiDCEurLAB__Display::GET $
, 	CursorPos = CursorPos $
...
PRO PMI__Button__RoiDCEurLAB__Display::SET, $
...
PRO PMI__Button__RoiDCEurLAB__Display::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Button__RoiDCEurLAB__Display::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Button__RoiDCEurLAB__Display__Event')
...
PRO PMI__Button__RoiDCEurLAB__Display__Define

	Struct = {PMI__Button__RoiDCEurLAB__Display 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
pro PMI__Button__Event__RoiDCEurLAB, ev

	PMI__Info, ev.top, Stdy=Stdy

    Series = Stdy->Names(0,ns,DefDim=3,ind=ind,sel=sel)
    Regions = Stdy->Names(1,nr)
    Units = ['Linear (a.u.)','Linear (%)','DSC-MRI','DCE@urLAB']
	...
	v = PMI__Form(ev.top, Title='Perfusion analysis setup', [$
		...
		ptr_new({Type:'DROPLIST',Tag:'units' , Label:'Signal model', Value:Units, Select:3}), $
	...
	PMI__Control, ev.top, Viewer = 'PMI__Button__RoiDCEurLAB__Display', Display=Display
...
pro PMI__Button__Control__RoiDCEurLAB, id, v
...
function PMI__Button__RoiDCEurLAB, parent,value=value, separator=separator
	...
	,	value = value	$
	,	event_pro = 'PMI__Button__Event__RoiDCEurLAB'	$
	,	pro_set_value = 'PMI__Button__Control__RoiDCEurLAB' $
	, 	separator = separator )
	return, id
end
