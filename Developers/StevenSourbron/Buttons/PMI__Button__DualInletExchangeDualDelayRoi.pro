pro PMI__Button__Event__DualInletExchangeDualDelayRoi, ev
	...
	Fit = FitDualInletDualDelay('FitDualInletExchange',time,curve,aif,vif,Pars=P,Delay=Pd,AIC=AIC,/noderivative,/quiet,constrained=1)

	PMI__Control, ev.top, Viewer = 'PMI__Display__PerfusionRoiOutput', Display=Display

	Display -> Set, /Refresh $
	,	Model = 'Dual-inlet exchange model with dual delay' $
	,	Time = Time $
	,	Curve = Curve $
	,	Fit = Fit $
	,	Units = Units $
...
pro PMI__Button__Control__DualInletExchangeDualDelayRoi, id, v
...
function PMI__Button__DualInletExchangeDualDelayRoi, parent,value=value, separator=separator

	ok=FitDualInletExchange()
	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit dual-inlet exchange model with dual delay (ROI)'

	id = widget_button(parent $
	,	value 		= value $
	,	event_pro 	= 'PMI__Button__Event__DualInletExchangeDualDelayRoi'$
	,	pro_set_value 	= 'PMI__Button__Control__DualInletExchangeDualDelayRoi' $
	, 	separator 	= separator	)
