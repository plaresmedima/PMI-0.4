pro PMI__Button__Event__DualInletUptakeDelayRoi, ev
	...
	Fit = FitDualInletDelay('FitDualInletUptake',time,curve,aif,vif,Pars=P,Delay=Pd,AIC=AIC,/noderivative,/quiet,/constrained)

	PMI__Control, ev.top, Viewer = 'PMI__Display__PerfusionRoiOutput', Display=Display

	Display -> Set, /Refresh $
	,	Model = 'Dual-inlet 2-compartment uptake model with delay' $
	,	Time = Time $
	,	Curve = Curve $
	,	Fit = Fit $
	,	Units = Units $
	...
	,	Parameters = $
		...
		,{Name:'Delay'					,Units:'sec'			,Value:1D*Pd							,Nr:11,Rnd:1} $
		,{Name:'Akaike Fit Error'		,Units:''				,Value:AIC								,Nr:14,Rnd:1} ]

end

pro PMI__Button__Control__DualInletUptakeDelayRoi, id, v
...
function PMI__Button__DualInletUptakeDelayRoi, parent,value=value, separator=separator

	ok = FitDualInletUptake()
	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit dual-inlet uptake model with delay (ROI)'

	id = widget_button(parent $
	,	value 		= value $
	,	event_pro 	= 'PMI__Button__Event__DualInletUptakeDelayRoi'$
	,	pro_set_value 	= 'PMI__Button__Control__DualInletUptakeDelayRoi' $
	, 	separator 	= separator	)
