PRO PMI__Button__FitSingleInletRoiIrene__Display::Fit
	...
	CASE Model OF
		...
		'1C Steady State':begin
			...
			Parameters = $
				[{Name:'PS = 0: Blood Volume'		,Units:'ml/100ml'		,Value:100D*P[0]/(1-self.Hematocrit)	,Nr: 2} $
				,{Name:'PS > 0: Plasma + Interstitial Volume'		,Units:'ml/100ml'		,Value:100D*P[0]	,Nr: 4} ]
			end
		...
		'Model-free':begin
			...
			Parameters = $
				...
				,{Name:'PS = 0: Blood MTT'			,Units:'sec'			,Value:1D*dt*total(IRF)/max(IRF)					,Nr: 1} $
				,{Name:'PS = 0: Blood Volume'		,Units:'ml/100ml'		,Value:100D*dt*total(IRF)/(1-self.Hematocrit)	,Nr: 2} $
				,{Name:'PS > 0: Plasma + Interstitial MTT'			,Units:'sec'			,Value:1D*dt*total(IRF)/max(IRF)		,Nr: 3} $
				,{Name:'PS > 0: Plasma + Interstitial Volume'		,Units:'ml/100ml'		,Value:100D*dt*total(IRF)	,Nr: 4} ]
			end
		...
		'Compartment':begin
			...
			Parameters = $
				...
				,{Name:'PS = 0: Blood MTT'			,Units:'sec'			,Value:1D*P[0]/P[1] ,Nr: 1} $
				,{Name:'PS = 0: Blood Volume'		,Units:'ml/100ml'		,Value:100D*P[0]/(1-self.Hematocrit)	,Nr: 2} $
				,{Name:'PS > 0: Plasma + Interstitial MTT'			,Units:'sec'			,Value:1D*P[0]/P[1] ,Nr: 3} $
				,{Name:'PS > 0: Plasma + Interstitial Volume'		,Units:'ml/100ml'		,Value:100D*P[0]	,Nr: 4} ]
			end
	...
	IF self.TissueT1 NE 0 THEN $
	Parameters = [Parameters,{Name:'Tissue T1', Units:'msec', Value:1000D*self.TissueT1, Nr:9} ]
...
PRO PMI__Button__FitSingleInletRoiIrene__Display::Plot
	...
	CASE OnDisplay OF
		...
		'VOF':begin
			Self -> GET, Time=Time, VofCurve=Y, VofName=VofName, Ytitle=Ytitle
			Self -> SET, /Erase
			if n_elements(Y) LE 1 then begin
				xyouts, x0, top-2*dy, 'Venous Outflow Function: <none>', color=0, /normal, charsize=1.5, charthick=1.5
				return
			endif
 			plot, time, Y, position=[0.1,0.2,0.5,0.9]  $
			, 	/xstyle, /ystyle $
			...
			, 	linestyle=0, thick=2 $
			, 	charsize=1.5, charthick=2.0, xthick=2.0, ythick=2.0
			xyouts, x0, top-2*dy, 'Venous Outflow Function: ' + VofName	, color=0, /normal, charsize=1.5, charthick=1.5
			end

		'FIT':BEGIN
			Self -> GET, RoiCurve=Curve, Time=Time, Fit=Fit, Model=Model, RoiName=RoiName, AifName=AifName, VofName=VofName, Ytitle=Ytitle
			Self -> SET, /Erase
...
FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::Event, ev

	Uname = widget_info(ev.id,/uname)

	i = where(Uname Eq ['ROI','AIF','VOF','FIT']+'bttn', cnt)
	If cnt eq 1 then begin
		Self->SET, OnDisplay=strmid(Uname,0,3), /Refresh
		return, 0B
	endif

	i = where(Uname Eq ['ROI','AIF','VOF'], cnt)
	If cnt eq 1 then begin
		case i OF
			0:index=0
			1:index=1
			2:index=3
		ENDCASE
		ptr_free, Self.Curve[index], Self.Curve[2], self.parameters
		if index eq 0 then self.TissueT1=0E
...
FUNCTION PMI__Button__FitSingleInletRoiIrene__Display__Event, ev

	widget_control, ev.handler, get_uvalue=self
	return, Self -> Event(ev)
END

FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::TissueT1

	If self.TissueT1 ne 0E then return, self.TissueT1
	Region = Self->GetRegion('ROI')
	PMI__Info, tlb(self.id), Stdy=Stdy
	Curve = PMI__RoiCurve(Stdy->DataPath(), Self.SatRec, Region, cnt=cnt)
	if cnt eq 0 then Curve = PMI__RoiCurveIndices(Stdy->DataPath(), Self.SatRec, Region, cnt=cnt)
	Time = [0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 1.2, 1.5, 2.0]
	Pars = [max(Curve),1.0] ;[Sinf, R1]
	Fit = mpcurvefit(Time, Curve, 1+0E*Curve, Pars, function_name='PMI__FitT1SaturationRecovery',/quiet,NODERIVATIVE=0)
	self.TissueT1 = 1/Pars[1]
	return, self.TissueT1
END

FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::Conc, Signal, List
	...
	if List Eq 'ROI' then T1 = Self->TissueT1() else T1=1.932
	relaxivity = 3.2
...
FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::GetRegion, List

	PMI__Info, tlb(self.id), Stdy=Stdy
	select = widget_info(widget_info(self.id,find_by_uname=List),/droplist_select)
	if List EQ 'VOF' then select=select-1
	if select GE 0 then return, Stdy->Obj(1,select)
	return, 0
END

FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::GetCurve, List
...
FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::GetName, List
...
PRO PMI__Button__FitSingleInletRoiIrene__Display::GET $
, 	CursorPos = CursorPos $
,	Model=Model, Positivity=Positivity, Delay=Delay $
,	Time=Time, Fit=Fit $
, 	RoiCurve=RoiCurve, AifCurve=AifCurve, VofCurve=VofCurve $
,	Roiname=Roiname, AifName=AifName, VofName=VofName $
, 	OnDisplay=OnDisplay, ConcModel=ConcModel, ytitle=Ytitle
	...
	if arg_present(Ytitle) then begin
		...
		Case C of
		0:Ytitle = 'Signal Enhancement (a.u.)'
		1:Ytitle = 'Relative Signal Enhancement (%)'
	...
	if arg_present(VofName) then VofName=Self->GetName('VOF')
	...
	if arg_present(VofCurve) then begin
		if ptr_valid(Self.Curve[3]) then begin
			VofCurve = *Self.Curve[3]
		endif else begin
			VofCurve = Self->GetCurve('VOF')
			if n_elements(VofCurve) GT 1 then Self.Curve[3] = ptr_new(VofCurve)
		endelse
		if n_elements(VofCurve) GT 1 then VofCurve = Self->Conc(VofCurve,'VOF')/(1-self.Hematocrit)
	endif
	...
	if arg_present(AifCurve) then begin
		...
		Self->Get, VofCurve=Vof
		If n_elements(Vof) gt 1 then begin ;correct partial volume
			self->get, time=t
			IRF = DeconvolveCurve(t,vof, AifCurve, dt=dt, pc='GCV', wm=1L, m0=0.001, m1=1.0, nm=100L, Quad='O2')
			AifCurve = AifCurve*dt*total(IRF)
		endif
	endif
...
PRO PMI__Button__FitSingleInletRoiIrene__Display::SET, $
	...
	Series=Series, SatRec = SatRec, Baseline=Baseline, Hematocrit=Hematocrit, $
	OnDisplay=OnDisplay
	...
	if n_elements(SatRec) 	ne 0 then Self.SatRec = SatRec
	...
	or n_elements(ysize) ne 0 then begin
		...
		xs = floor((xsize - 500)/5E)
		if xs lt 50 then xs = 50
		List = ['ROI','AIF','VOF','FIT','SIG']
...
PRO PMI__Button__FitSingleInletRoiIrene__Display::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Button__FitSingleInletRoiIrene__Display::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Button__FitSingleInletRoiIrene__Display__Event')
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
  			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['Model-free', '1C Steady State','Compartment', 'Patlak','Modified Tofts','2C Uptake','2C Exchange'])
...
PRO PMI__Button__FitSingleInletRoiIrene__Display__Define

	Struct = {PMI__Button__FitSingleInletRoiIrene__Display 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
	,	Curve:ptrarr(4) $ ;RoiCurve, AifCurve, Fit, VofCurve
	,	TissueT1: 0E $
	,	Parameters: ptr_new() $
	,	SatRec: obj_new() $
	,	Series: obj_new() $
...
FUNCTION PMI__Button__Input__FitSingleInletRoiIrene, top, series, SatRec, in

    PMI__Info, top, Stdy=Stdy
    DynSeries = Stdy->Names(0,DefDim=3,ind=ind,sel=sel)
	in = {ser:sel, sar:sel, aif:stdy->sel(1), roi:stdy->sel(1), vof:0, nb:10, hct:0.45}
	...
	WHILE 1 DO BEGIN
		...
		ptr_new({Type:'DROPLIST',Tag:'sar', Label:'Saturation-recovery data', Value:DynSeries, Select:in.sar}), $
...
pro PMI__Button__Event__FitSingleInletRoiIrene, ev

	IF NOT PMI__Button__Input__FitSingleInletRoiIrene(ev.top, series, SatRec, in) THEN return

	PMI__Control, ev.top, Viewer = 'PMI__Button__FitSingleInletRoiIrene__Display', Display=Display
	...
	Display -> Set, /Refresh, $
		...
		Baseline = in.nb, $
		Hematocrit = in.hct, $
		set_droplist_select = [in.roi,in.aif,in.vof]
end

pro PMI__Button__Control__FitSingleInletRoiIrene, id, v
...
function PMI__Button__FitSingleInletRoiIrene, parent,value=value, separator=separator
	...
	,	value = value	$
	,	event_pro = 'PMI__Button__Event__FitSingleInletRoiIrene'	$
	,	pro_set_value = 'PMI__Button__Control__FitSingleInletRoiIrene' $
	, 	separator = separator )
	return, id
end
