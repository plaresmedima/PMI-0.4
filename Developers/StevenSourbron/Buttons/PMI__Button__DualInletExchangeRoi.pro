pro PMI__Button__Event__DualInletExchangeRoi, ev
	...
	Fit = FitDualInletExchange(time,curve,aif,vif,Pars=P,AIC=AIC,/noderivative,/quiet,constrained=1)

	PMI__Control, ev.top, Viewer = 'PMI__Display__PerfusionRoiOutput', Display=Display

	Display -> Set, /Refresh $
	,	Model = 'Dual-inlet exchange model' $
	,	Time = Time $
	,	Curve = Curve $
	,	Fit = Fit $
	,	Units = Units $
...
pro PMI__Button__Control__DualInletExchangeRoi, id, v
...
function PMI__Button__DualInletExchangeRoi, parent,value=value, separator=separator

	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit dual-inlet exchange model (ROI)'

	id = widget_button(parent $
	,	value 		= value $
	,	event_pro 	= 'PMI__Button__Event__DualInletExchangeRoi'$
	,	pro_set_value 	= 'PMI__Button__Control__DualInletExchangeRoi' $
	, 	separator 	= separator	)
