...
FUNCTION PMI__Button__Input__sohaibDistributedParameter, top, series, aif, roi, in
...
pro PMI__Button__Event__sohaibDistributedParameter, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__sohaibDistributedParameter(ev.top,series,aif,roi,in) THEN RETURN
 		...
 		if cnt gt 0 then begin
			...
			if cnt gt 0 then begin
				...
				for r=0L,cnt-1 do begin
					...
					Fit = FitSingleInlet('DistributedParameter',time, aif, curve, Par, /noderivative, /positivity)
...
pro PMI__Button__Control__sohaibDistributedParameter, id, v
...
function PMI__Button__sohaibDistributedParameter, parent,value=value,separator=separator

	SingleInletDistributedParameter

    if n_elements(value) eq 0 then value = 'DP analysis (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__sohaibDistributedParameter' $
    ,	pro_set_value = 'PMI__Button__Control__sohaibDistributedParameter' $
    ,  	separator = separator )
