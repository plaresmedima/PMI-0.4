...
FUNCTION PMI__Button__Input__SohaibDeconvolution, top, series, aif, roi, in
...
pro PMI__Button__Event__SohaibDeconvolution, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__SohaibDeconvolution(ev.top,series,aif, roi, in) THEN RETURN
    ...
    Sbf = Stdy->New('SERIES', Domain= Dom,  Name= 'Model-free: Blood Flow (ml/min/g)' )
...
pro PMI__Button__Control__SohaibDeconvolution, id, v
...
function PMI__Button__SohaibDeconvolution, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'Fast deconvolution analysis (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__SohaibDeconvolution' $
    ,	pro_set_value = 'PMI__Button__Control__SohaibDeconvolution' $
    ,  	separator = separator )
