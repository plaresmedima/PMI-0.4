...
FUNCTION PMI__Button__Input__sohaibcompartmentlinearconc, top, series, aif, roi, in
...
pro PMI__Button__Event__sohaibcompartmentlinearconc, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__sohaibcompartmentlinearconc(ev.top,series,aif,roi,in) THEN RETURN

	PMI__Message, status, 'Calculating..'

	Dom = {z:Series->z(), t:Series->t(0), m:Series->m()}

    Sev = Stdy->New('SERIES', Domain= Dom,  Name= '1C-Gd: Extracellular Volume (ml/100ml)' )
    Stt = Stdy->New('SERIES', Domain= Dom,  Name= '1C-Gd: Mean Transit Time (sec)' )
    Sbf = Stdy->New('SERIES', Domain= Dom,  Name= '1C-Gd: Blood Flow (ml/min/g)' )
...
pro PMI__Button__Control__sohaibcompartmentlinearconc, id, v
...
function PMI__Button__sohaibcompartmentlinearconc, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'One-compartment analysis (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__sohaibcompartmentlinearconc' $
    ,	pro_set_value = 'PMI__Button__Control__sohaibcompartmentlinearconc' $
    ,  	separator = separator )
