pro PMI__Button__Event__TMUH_LiverDCE_SeriesMerge, ev
...
;RESCALE
    ...
    Result = Stdy -> New('SERIES', Default=Rec, Name='3D-dynamic' )
    Result->m, d[0:1]

    for k=0L,d[2]*d[3]-1 do begin

       PMI__Message, status, 'Resizing', k/(d[2]*d[3]-1E)
       im = Rec -> Read(Stdy->DataPath(),k)
       im = congrid(im,d[0],d[1],/interp)
...
;Clean up
	...
	nSeries = Stdy->n(0)
	Stdy -> Delete, 0, [nSeries-3,nSeries-2]
	Stdy -> Sel, 0, nSeries-3
...
pro PMI__Button__Control__TMUH_LiverDCE_SeriesMerge, id, v
...
function PMI__Button__TMUH_LiverDCE_SeriesMerge, parent, separator=separator, value=value
	...
	, 	separator	= separator $
	, 	value 		= value	$
	,	event_pro 	= 'PMI__Button__Event__TMUH_LiverDCE_SeriesMerge' $
    ,	pro_set_value 	= 'PMI__Button__Control__TMUH_LiverDCE_SeriesMerge' )
