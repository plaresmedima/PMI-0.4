...
PRO PMI__Button__Display__HEPARIM_PILOT_FitDualInletRoi::Fit
	...
	;correction factor for difference in relaxivity
	relaxivity_corr = self.relaxivity_hepatocytes/self.relaxivity

	CASE Model OF

		'Uptake':BEGIN
			P = [FA,FV,0.2,0.2] ;P = [FA,FV,VE,E]
		...
		'Uptake with relaxivity correction':BEGIN
			P = [FA,FV,19.2,0.2] ;P = [FA,FV,TE,Eapp]
			Fit = FitDualInlet('UptakeGadoxetate', Time, Aif, Vif, Curve, P, LIMITED_ABOVE=[1,1,1,1], LIMITS_ABOVE=[1000/6000D,1000/6000D,100,1D], $
			  DELAY_PAR=Pd, DELAY_VALUES=DELAY_VALUES, DELAY_WHICH=DELAY_WHICH, AKAIKE_ERROR=aic, /POSITIVITY, /NODERIVATIVE, FIXED=fixed)
			E = P[3]/relaxivity_corr
			Parameters = $
				[{Name:'Extracellular Volume'	,Units:'ml/100ml'		,Value:100D*(P[0]+P[1])*P[2]/(1-E)	,Nr: 4} $
				,{Name:'Akaike Fit Error'		,Units:''				,Value:AIC							,Nr:16} $
				,{Name:'Extracellular MTT'		,Units:'sec'			,Value:1D*P[2]						,Nr: 3} $
				,{Name:'Uptake Fraction'		,Units:'%' 				,Value:100D*E						,Nr:11} $
				,{Name:'Uptake Rate'			,Units:'/100/min'		,Value:6000D*(P[0]+P[1])*E/(1-E)	,Nr:12} ]
			IF keyword_set(AIF_Use) THEN 	Parameters = [Parameters $
				,{Name:'Arterial Plasma Flow'	,Units:'ml/100ml/min'	,Value:6000*P[0]					,Nr: 0} ]
			IF keyword_set(VIF_Use) THEN 	Parameters = [Parameters $
				,{Name:'Venous Plasma Flow'		,Units:'ml/100ml/min'	,Value:6000*P[1]				,Nr: 1} ]
...
PRO PMI__Button__Display__HEPARIM_PILOT_FitDualInletRoi::SET, $
	...
	T1t=T1t, T1a=T1a, T1v=T1v, TR=TR, FA=FA, $
	OnDisplay=OnDisplay
	...
	if n_elements(Relaxivity_blood) 	ne 0 then self.Relaxivity = Relaxivity_blood
	...
	self.DrawId	= widget_draw(self.id,/retain)
		...
		Base = widget_base(Controls,/row,/frame,/base_align_center)
			id = widget_button(Base, xsize=25, ysize=19, value='FIT', uname='FITbttn')
  			id = widget_droplist(Base,/dynamic_resize, uname='FIT',value = ['Uptake','Uptake with relaxivity correction'])
...
pro PMI__Button__Event__HEPARIM_PILOT_FitDualInletRoi, ev
	...
	v = PMI__Form(ev.top, Title='Perfusion analysis setup', [$
		...
		ptr_new({Type:'VALUE'	,Tag:'nt' , Label:'Length of baseline (nr of time points)', Value:15L}),$
	...
	CASE FieldStrength OF
		...
		3.0: BEGIN
			...
			T1t = 812E ;Stanisz et al
			relaxivity_hepatocytes = 9.8 ;Ziemian et al
			relaxivity_blood = 6.4 ;Ziemian et al
		END
		ELSE: BEGIN
			ok = dialog_message('Unknown field strength ('+strcompress(FieldStrength,/remove_all)+' T)')
			RETURN
		END
	ENDCASE

;INCORRECT FOR 2D SEQUENCE !!!!!!!!!!!!!!!!!
