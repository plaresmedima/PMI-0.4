Pro PMI__Button__Event__DualInletExchangeUptakeDelayRoi, ev
	...
	Fit = FitDualInletDelay('FitDualInletExchangeUptake',time,curve,aif,vif,Pars=P,Delay=Pd,AIC=AIC,/noderivative,/quiet,/constrained)

	PMI__Control, ev.top, Viewer='PMI__Display__PerfusionRoiOutput', Display=Display

	Display -> Set, /Refresh $
	,	Model = 'Dual-inlet exchange-uptake model with delay' $
	,	Time = Time $
	,	Curve = Curve $
	,	Fit = Fit $
	,	Units = Units $
	...
	,	Parameters = $
		...
		,{Name:'Arterial Flow Fraction'			,Units:'%' 				,Value:100D*P[0]/(P[0]+P[1])	,Nr: 2,Rnd:1} $
...
pro PMI__Button__Control__DualInletExchangeUptakeDelayRoi, id, v
...
function PMI__Button__DualInletExchangeUptakeDelayRoi, parent,value=value, separator=separator

	ok=FitDualInletExchangeUptake()
	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit dual-inlet exchange-uptake model with delay (ROI)'

	id = widget_button(parent $
	,	value 		= value $
	,	event_pro 	= 'PMI__Button__Event__DualInletExchangeUptakeDelayRoi'$
	,	pro_set_value 	= 'PMI__Button__Control__DualInletExchangeUptakeDelayRoi' $
	, 	separator 	= separator	)
