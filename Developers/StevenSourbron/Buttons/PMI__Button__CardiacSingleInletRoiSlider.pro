PRO PMI__Display__CardiacSingleInletRoiSlider::Fit
...
FUNCTION PMI__Display__CardiacSingleInletRoiSlider::Event, ev
...
FUNCTION PMI__Display__Event__CardiacSingleInletRoiSlider, ev
...
FUNCTION PMI__Display__CardiacSingleInletRoiSlider::GetCurve, Region
...
FUNCTION PMI__Display__CardiacSingleInletRoiSlider::GetName, Region
...
PRO PMI__Display__CardiacSingleInletRoiSlider::GET, $
 	CursorPos = CursorPos, $
	...
	if arg_present(ytitle) then if self.relative then ytitle='S/S0-1' else ytitle='S-S0'
	...
	if arg_present(RoiCurve) then $
		if ptr_valid(Self.Curve[0]) then RoiCurve = *Self.Curve[0] $
			...
			if self.relative then RoiCurve=RoiCurve/S0-1 else RoiCurve=RoiCurve-S0
	...
	if arg_present(AifCurve) then $
		if ptr_valid(Self.Curve[1]) then AifCurve = *Self.Curve[1] $
			...
			if self.relative then AifCurve=AifCurve/S0-1 else AifCurve=AifCurve-S0
			AifCurve = AifCurve/0.55
...
PRO PMI__Display__CardiacSingleInletRoiSlider::SET, $
...
PRO PMI__Display__CardiacSingleInletRoiSlider::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Display__CardiacSingleInletRoiSlider::Init, parent, CursorPos, xsize=xsize, ysize=ysize, $
	Series = Series, $
	Baseline = nbase, $
	Relative = rel, $
	...
	self.id = widget_base(parent,/column,event_func='PMI__Display__Event__CardiacSingleInletRoiSlider')
...
PRO PMI__Display__CardiacSingleInletRoiSlider__Define
	...
	Struct = {PMI__Display__CardiacSingleInletRoiSlider, 	$
		id: 0L, 	$
		DrawId: 0L, $
		CursorPos:lonarr(4),	$
...
pro PMI__Button__Event__CardiacSingleInletRoiSlider, ev
	...
	Display = PMI__DisplayNew(ev.top, 'PMI__DISPLAY__CARDIACSINGLEINLETROISLIDER',$
...
pro PMI__Button__Control__CardiacSingleInletRoiSlider, id, v
...
function PMI__Button__CardiacSingleInletRoiSlider, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Single-inlet models with variable time-window (ROI)'

	return, widget_button(parent, $
		value = value,	$
		event_pro = 'PMI__Button__Event__CardiacSingleInletRoiSlider',	$
		pro_set_value = 'PMI__Button__Control__CardiacSingleInletRoiSlider', $
	 	separator = separator )
end
