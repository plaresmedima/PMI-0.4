...
FUNCTION PMI__Button__Input__SohaibDeconvolutionConc, top, series, aif, roi, in
...
pro PMI__Button__Event__SohaibDeconvolutionConc, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__SohaibDeconvolutionConc(ev.top,series,aif, roi, in) THEN RETURN

	PMI__Message, status, 'Preparing calculation..'

	Dom = {z:Series->z(), t:Series->t(0), m:Series->m()}

    Svd = Stdy->New('SERIES', Domain= Dom,  Name= 'Model-free-Gd: Extracellular Volume (ml/100ml)')
    Stt = Stdy->New('SERIES', Domain= Dom,  Name= 'Model-free-Gd: Mean Transit Time (sec)' )
    Sbf = Stdy->New('SERIES', Domain= Dom,  Name= 'Model-free-Gd: Blood Flow (ml/min/g)' )
	...
	Aif = Concentration_SR_SpGRE(Aif, Aif0, in.T1b, relaxivity, TD, FA, TR, Nphase)/Hct
		...
		if cnt gt 0 then begin
			...
			if cnt gt 0 then begin
    			...
    			P0 = P0[nozero]
 				...
 				for r=0L,cnt-1 do P[r,*] = Concentration_SR_SpGRE(reform(P[r,*]), P0[r], in.T1t, relaxivity, TD, FA, TR, Nphase)
				...
				BF = fltarr(d[0]*d[1]) & BF[k[nozero]] = 60*remove_inf(max(P,dimension=2))/0.55
    			VD = fltarr(d[0]*d[1]) & VD[k[nozero]] = 100*remove_inf((t[1]-t[0])*total(P,2))
    			TT = fltarr(d[0]*d[1]) & TT[k[nozero]] = 60E*remove_inf(VD[k[nozero]]/BF[k[nozero]])/0.55
...
pro PMI__Button__Control__SohaibDeconvolutionConc, id, v
...
function PMI__Button__SohaibDeconvolutionConc, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'Fast deconvolution analysis (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__SohaibDeconvolutionConc' $
    ,	pro_set_value = 'PMI__Button__Control__SohaibDeconvolutionConc' $
    ,  	separator = separator )
