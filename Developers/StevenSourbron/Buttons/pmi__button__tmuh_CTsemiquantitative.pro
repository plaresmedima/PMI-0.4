pro PMI__Button__Event__TMUH_CTSemiQuantitative, ev
	...
	, ptr_new({Type:'VALUE'	,Tag:'nb',Label: 'Length of baseline (# of dynamics)', Value:5L}) ] )
...
pro PMI__Button__Control__TMUH_CTSemiQuantitative, id, v
...
function PMI__Button__TMUH_CTSemiQuantitative, parent,value=value, separator=separator

	if n_elements(value) eq 0 then value = 'Semi-quantitative parameters (Pixel)'

    id = widget_button(parent $
    , 	value 		= value $
	, 	event_pro	= 'PMI__Button__Event__TMUH_CTSemiQuantitative' $
	,	pro_set_value 	= 'PMI__Button__Control__TMUH_CTSemiQuantitative' $
	, 	separator 	= separator	)
