pro PMI__Button__Event__KidneyFiltrationRoi, ev

	PMI__Info, ev.top, Status=StatusId, Stdy=Stdy
    DynSeries = Stdy->Names(0,DefDim=3,ind=ind,sel=sel)

	in = PMI__Form(ev.top, Title='Filtration model', [$
	...
	Fit = FitSingleInlet('Filtration', Time-Time[0], Aif, Roi, P, DELAY_PAR=Pd, DELAY_VALUES=[0,10,0.5], /POSITIVITY, LIMITED_ABOVE=[0,0,1,1])
...
pro PMI__Button__Control__KidneyFiltrationRoi, id, v

	PMI__Info, tlb(id), Stdy=Stdy
	if obj_valid(Stdy) then begin
		Series = Stdy->Names(0,ns,DefDim=3)
		Regions = Stdy->Names(1,nr)
		sensitive = (ns gt 0) and (nr ge 2)
	endif else sensitive=0
    widget_control, id, sensitive=sensitive
end

function PMI__Button__KidneyFiltrationRoi, parent, value=value, separator=separator

	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Kidney protocol without triggering (ROI)'

	return, widget_button(parent, $
		value 		= value, $
		event_pro 	= 'PMI__Button__Event__KidneyFiltrationRoi',	$
		pro_set_value = 'PMI__Button__Control__KidneyFiltrationRoi', $
	 	separator 	= separator	)
