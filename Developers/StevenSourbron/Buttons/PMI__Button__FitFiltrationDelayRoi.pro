pro PMI__Button__Event__FitFiltrationDelayRoi, ev
  ...
  if not TumorRoiAnalysis__input($
	...
	Fit = FitSingleInlet('Filtration', time, aif, curve, P, DELAY_PAR=Pd, DELAY_VALUES=[0,20,time[1]/2], AKAIKE_ERROR=aic, /POSITIVITY, LIMITED_ABOVE=[1,0,1,1])

	PMI__Control, ev.top, Viewer = 'PMI__Display__PerfusionRoiOutput', Display=Display

	Display -> Set, /Refresh $
	,	Model = 'Two-compartment filtration model with delay' $
	,	Time = Time $
	,	Curve = Curve $
	,	Fit = Fit $
	,	Units = Units $
	...
	,	Parameters = $
		...
		,{Name:'ESS MTT'		,Units:'sec'			,Value:1D*P[0]*P[2]/(P[1]*P[3])	,Nr: 3,Rnd:1} $
...
pro PMI__Button__Control__FitFiltrationDelayRoi, id, v
...
function PMI__Button__FitFiltrationDelayRoi, parent,value=value, separator=separator

	SingleInletFiltration
	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit 2-compartment filtration model with delay (ROI)'

	id = widget_button(parent 						$
	,	value 		= value		$
	,	event_pro 	= 'PMI__Button__Event__FitFiltrationDelayRoi'	$
	,	pro_set_value 	= 'PMI__Button__Control__FitFiltrationDelayRoi' $
	, 	separator 	= separator						)
	return, id
end
