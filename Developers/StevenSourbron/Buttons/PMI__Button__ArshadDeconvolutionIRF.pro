FUNCTION PMI__Button__Input__ArshadDeconvolutionIRF, top, series, aif, roi, in

    PMI__Info, top, Status=Status, Stdy=Stdy
    DynSeries = Stdy->Names(0,DefDim=3,ind=ind,sel=sel)
	in = {ser:sel, aif:stdy->sel(1), roi:0, nb:2}
...
pro PMI__Button__Event__ArshadDeconvolutionIRF, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
    IF NOT PMI__Button__Input__ArshadDeconvolutionIRF(ev.top,series,aif,roi,in) THEN RETURN

	Dom = {z:Series->z(), t:Series->t(0), m:Series->m()}
    Spf = Stdy->New('SERIES', Domain= Dom,  Name= Series->name() +'__Blood Flow (ml/100ml/min)' )
    Svd = Stdy->New('SERIES', Domain= Dom,  Name= Series->name() +'__Extracellular Volume (ml/100ml)')
    Stt = Stdy->New('SERIES', Domain= Dom,  Name= Series->name() +'__Mean Transit Time (sec)' )
    Res = Stdy->New('SERIES', Default=Series,  Name= Series->name() +'__Impulse Response (ml/100ml/min)' )
...
;			InvertIllPosedFast, P, (t[1]-t[0])*convolution_matrix(a), REGPAR=0.15
	    	INVERTILLPOSED, P, (t[1]-t[0])*convolution_matrix(a), pc='LCC', wm=9L, ws=1L, m0=0.001, m1=1.0, nm=100L
			...
			for l=0L,d[3]-1 do begin
				slice = slice*0
				slice[k] = 6000*P[*,l]/0.55
				Res->Write, Stdy->DataPath(), slice, j, l
			endfor
		endif
	endfor
...
pro PMI__Button__Control__ArshadDeconvolutionIRF, id, v
...
function PMI__Button__ArshadDeconvolutionIRF, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'Deconvolution analysis (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__ArshadDeconvolutionIRF' $
    ,	pro_set_value = 'PMI__Button__Control__ArshadDeconvolutionIRF' $
    ,  	separator = separator )
