...
PRO MOCOMO_2D_INTERACTING_LINEAR__TEST, SETTINGS=settings
...
;MOCOMO SETTINGS
  ...
  CASE settings OF
    ...
    4:BEGIN
      ...
      w = window_slice
      filename = 'wholeslice'
      Modelname = 'TwoCompartmentFiltrationDelay'
    END
  ENDCASE
...
;OPTIMIZE
  ...
  MOCOMO_2D_INTERACTING_LINEAR, Source, ModelName, Independent, GRID_SIZE=Resolution, TOLERANCE=Precision, WINDOW=w
...
;EXPORT DEFORMED GIF
  ...
  image = BYTE(255*(Source-min(Source))/(0.5*max(Source)-min(Source)))
  File = DataPath + filename + '_Interacting_[' + time + ' min, ' + improvement + ' %]' + '.gif'
  LOADCT, 0
  FOR k=0L,ns[0]-1 DO write_gif, File, REFORM(image[k,*,*]), /MULTIPLE
  write_gif, File, /CLOSE
