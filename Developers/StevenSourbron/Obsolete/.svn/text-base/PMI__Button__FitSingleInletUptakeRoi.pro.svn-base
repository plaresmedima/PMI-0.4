pro PMI__Button__Event__FitSingleInletUptakeRoi, ev
	...
	,	Parameters = $
		...
		,{Name:'Extraction Flow'	,Units:'ml/100ml/min'	,Value:6000D*P[1]*P[2]/(1-P[2])	,Nr: 4,Rnd:1} $
		,{Name:'Exchange Fraction'	,Units:'%'				,Value:100D*P[2]	,Nr: 5,Rnd:1} $
		,{Name:'Akaike Fit Error'	,Units:''				,Value:AIC			,Nr:14,Rnd:1} ]

end

pro PMI__Button__Control__FitSingleInletUptakeRoi, id, v
...
function PMI__Button__FitSingleInletUptakeRoi, parent,value=value, separator=separator

	PMI__Display__PerfusionRoiOutput__Define

	if n_elements(value) eq 0 then value = 'Fit single-inlet uptake model (ROI)'

	id = widget_button(parent $
	,	value 		= value $
	,	event_pro 	= 'PMI__Button__Event__FitSingleInletUptakeRoi'$
	,	pro_set_value = 'PMI__Button__Control__FitSingleInletUptakeRoi' $
	, 	separator 	= separator	)
