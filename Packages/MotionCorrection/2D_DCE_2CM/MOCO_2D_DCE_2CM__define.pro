;PRO MOCO_2D_DCE_2CM::LLS_2CFM_FIT, S
;
;    IF norm(S) EQ 0. THEN RETURN
;
;    n = n_elements(S)
;    X = *self.independent
;    t = X[0:n-1]
...
;    S1 = [0, TOTAL( dt*(S[0:n-2]+S[1:n-1]) , /cumulative)]
;    S2 = [0, TOTAL( dt*(S1[0:n-2]+S1[1:n-1]) , /cumulative)]
;
;	weight = 1/t2[1:*]
;
;	Sw = S[1:*]*weight
...
;	t2 = t2[1:*]*weight
;
;	A = TRANSPOSE([[-S2],[-S1],[ca1],[ca2],[-t2]])
;	SVDC, A, W, U, V
;	X = TRANSPOSE(U) ## TRANSPOSE([Sw])
...
;END
;
;PRO MOCO_2D_DCE_2CM::SIGNAL_FIT_2CFM, r
;
...
;END
;
;PRO MOCO_2D_DCE_2CM::LLS_2CFM_FIT_PRECOMPUTE
;
;	X = *self.Independent
;
;	n = (n_elements(X)-1)/2
...
;    t1 = [0, TOTAL( dt*(t[0:n-2]+t[1:n-1]) , /cumulative)]
;    t2 = [0, TOTAL( dt*(t1[0:n-2]+t1[1:n-1]) , /cumulative)]
;
;    *self.independent = [t,t2,ca1,ca2]
;
;END

FUNCTION MOCO_2D_DCE_2CM::LLS_2CFM_FIT, ct
...
FUNCTION MOCO_2D_DCE_2CM::ffd, D
...
FUNCTION MOCO_2D_DCE_2CM::FFD_GRAD
...
FUNCTION MOCO_2D_DCE_2CM::FFD_LSEARCH
...
FUNCTION MOCO_2D_DCE_2CM::FFD_REG, t
...
PRO MOCO_2D_DCE_2CM::ffd_reg_precompute
...
PRO MOCO_2D_DCE_2CM::INITIALISE
...
FUNCTION MOCO_2D_DCE_2CM::DEFORMED

  self -> LLS_2CFM_FIT_PRECOMPUTE
...
PRO MOCO_2D_DCE_2CM__DEFINE

  struct = {MOCO_2D_DCE_2CM, $
  	source: ptr_new(), $
  	Independent: ptr_new(), $
  	DefField: ptr_new(), $
  	Deformed: ptr_new(), $
