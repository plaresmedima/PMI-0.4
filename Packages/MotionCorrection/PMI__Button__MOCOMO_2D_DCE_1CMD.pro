...
tt = systime(1)
    ...
    for k=0L,d[2]-1 do begin
    	...
    	if product(win[k].n) gt 0 then begin
    		...
    		MOCOMO_2D_DCE_1CMD = OBJ_NEW('MOCOMO_2D_DCE_1CMD', ptr_new(Source), in.res, in.prec, [Time, aif, in.nb], Win=win[k])
    		Source = TRANSPOSE(MOCOMO_2D_DCE_1CMD->deformed(), [1,2,0])
    		OBJ_DESTROY, MOCOMO_2D_DCE_1CMD
		endif
