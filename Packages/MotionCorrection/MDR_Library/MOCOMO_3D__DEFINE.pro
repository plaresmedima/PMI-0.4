FUNCTION MOCOMO_3D::SCALEUP, deformation_field, grid_size
  ...
  Z = dr[2] * findgen(grid_size[2])

  RETURN, INTERPOLATE(deformation_field, X, Y, Z, /GRID)
END



FUNCTION MOCOMO_3D::NODEFORMATION, nt, grid_size
  ...
  FOR k=0L, grid_size[2]-1 DO BEGIN
    rb[0,i,j,k] = self.win[0] + db[0]*i
    rb[1,i,j,k] = self.win[1] + db[1]*j
    rb[2,i,j,k] = self.win[2] + db[2]*k
  ENDFOR
  ENDFOR
  ENDFOR
...
PRO MOCOMO_3D::FFD, F, S, D, xc, yc, zc

  n = size(D, /Dimensions)
  IF N_ELEMENTS(F) EQ 0 THEN F = FLTARR(n[0], N_ELEMENTS(xc), N_ELEMENTS(yc), N_ELEMENTS(zc), /NOZERO)

  FOR k=0L,n[0]-1 DO BEGIN

    xk = INTERPOLATE(D[k,0,*,*,*], xc, yc, zc, /GRID)
    yk = INTERPOLATE(D[k,1,*,*,*], xc, yc, zc, /GRID)
    zk = INTERPOLATE(D[k,2,*,*,*], xc, yc, zc, /GRID)

    F[k,*,*,*] = INTERPOLATE(S[k,*,*,*], xk, yk, zk)

  ENDFOR
END


FUNCTION MOCOMO_3D::CHISQ, deformed, target

  RETURN, 0.5*total((deformed - Target)^2.)
END


FUNCTION MOCOMO_3D::LINESEARCH, Grad, Source, deformed, deformation_field, target, xc, yc, zc, STEPSIZE=stepsize
  ...
  WHILE 1 DO BEGIN
	...
	self->FFD, D_try, Source, F_try, xc, yc, zc
...
PRO MOCOMO_3D::GRADIENT, Gradient, Sx, Sy, Sz, residue, deformation_field, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val

;Deform the image gradient

  x = INTERPOLATE(deformation_field[*,0,*,*,*], xc, yc, zc, /GRID)
  y = INTERPOLATE(deformation_field[*,1,*,*,*], xc, yc, zc, /GRID)
  z = INTERPOLATE(deformation_field[*,2,*,*,*], xc, yc, zc, /GRID)

  Dx = INTERPOLATE(Sx, long(x), y, z)
  Dy = INTERPOLATE(Sy, x, long(y), z)
  Dz = INTERPOLATE(Sz, x, y, long(z))
...
PRO MOCOMO_3D::WEIGHTS, nD, nw, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val
  ...
  FOR k=0L, nw[2]-1 DO BEGIN
    rp[0,i,j,k] = xc[i] ;X-coordinate of pixel (i,j,k)
    rp[1,i,j,k] = yc[j] ;Y-coordinate of pixel (i,j,k)
    rp[2,i,j,k] = zc[k] ;Z-coordinate of pixel (i,j,k)
  ENDFOR
  ENDFOR
  ENDFOR
...
;	Interpolate the mask on those pixels (Wi)
  ...
  FOR i=0L,NumberOfControlPoints-1 DO BEGIN
	...
	; Wi_cnt = number of pixels under the mask
  	Wi_loc = where((abs(xi) lt 1) and (abs(yi) lt 1) and (abs(zi) lt 1), Wi_cnt)

	; Interpolate the mask on those locations
  	Wi = INTERPOLATE(Mask, 1+xi[Wi_loc], 1+yi[Wi_loc], 1+zi[Wi_loc])
...
FUNCTION MOCOMO_3D::FIT, deformed, deformation_field, Source, Target, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val
  ...
  Sz = Source[*,*,*,1:ns[3]-1] - Source[*,*,*,0:ns[3]-2]
  ...
  FOR steps=0L, self.max_number_of_gradient_descent_steps DO BEGIN
    ...
    self->GRADIENT, Gradient, Sx, Sy, Sz, target-deformed, deformation_field, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val
    IF self->LINESEARCH(Gradient, Source, deformed, deformation_field, Target, xc, yc, zc, STEPSIZE=stepsize) THEN BREAK
  ENDFOR
...
FUNCTION MOCOMO_3D::FIT_ALL, deformed, deformation_field, Source, Target, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val
   ...
   FOR t=0L,ns[0]-1 DO BEGIN
     ...
     converged *= self->FIT(d_t, f_t, Source[t,*,*,*], Target[t,*,*,*], xc, yc, zc, Weight_cnt, Weight_loc, Weight_val)
...
PRO MOCOMO_3D::OPTIMIZE, deformed, deformation_field, Source

  nd = SIZE(deformed, /DIMENSIONS)
  nf = SIZE(deformation_field, /DIMENSIONS)

  self->WEIGHTS, nf[2:*], nd[1:*], xc, yc, zc, Weight_cnt, Weight_loc, Weight_val
...
print, 'iteration', updates
    ...
    no_change = self->FIT_ALL(deformed, deformation_field, source, target, xc, yc, zc, Weight_cnt, Weight_loc, Weight_val)
...
PRO MOCOMO_3D::MULTIRESOLUTION, Source, Deformed, deformation_field
...
print, 'resolution', resolution_level
...
PRO MOCOMO_3D::APPLY, Source, deformation_field

  xw = self.win[0] + LINDGEN(self.win[3]) ;x-coordinates of window pixels
  yw = self.win[1] + LINDGEN(self.win[4]) ;y-coordinates of window pixels
  zw = self.win[2] + LINDGEN(self.win[5])
...
FUNCTION MOCOMO_3D::INIT, Source, ModelName, Independent, $
  ...
  self.win = [win.p[0],win.p[1],win.p[2],win.n[0],win.n[1],win.n[2]]
  self.model = OBJ_NEW('MoCoModel_' + ModelName, Independent)
  self.max_number_of_parameter_updates = 10
  self.max_number_of_gradient_descent_steps = 100
...
PRO MOCOMO_3D__DEFINE
  ...
  struct = {MOCOMO_3D $
    ...
    , win: LONARR(6) $
    , model: obj_new() $
    , max_number_of_parameter_updates: 10L $
    , max_number_of_gradient_descent_steps: 100 $
    }
