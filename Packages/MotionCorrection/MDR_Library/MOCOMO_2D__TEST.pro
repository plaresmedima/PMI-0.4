...
PRO MOCOMO_2D__TEST, SETTINGS=settings
...
;OPTIMIZE
  ...
  MOCOMO_2D, Source, ModelName, Independent, GRID_SIZE=Resolution, TOLERANCE=Precision, WINDOW=w
...
;EXPORT DEFORMED GIF
  ...
  image = BYTE(255*(Source-min(Source))/(0.5*max(Source)-min(Source)))
  File = DataPath + filename + '_sequential_[' + time + ' min, ' + improvement + ' %]' + '.gif'
  LOADCT, 0
  FOR k=0L,ns[0]-1 DO write_gif, File, REFORM(image[k,*,*]), /MULTIPLE
  write_gif, File, /CLOSE
