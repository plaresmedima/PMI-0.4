...
FUNCTION PMI__Button__Input__MOCO_2D_DCE_1CM, top, series, aif, in, Win
...
pro PMI__Button__Event__MOCO_2D_DCE_1CM, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy

    IF NOT PMI__Button__Input__MOCO_2D_DCE_1CM(ev.top,series,aif,in,win) THEN RETURN
...
tt = systime(1)
    ...
    for k=0L,d[2]-1 do begin
    	...
    	if product(win[k].n) gt 0 then begin
    		...
    		MOCO_2D_DCE_1CM = OBJ_NEW('MOCO_2D_DCE_1CM', ptr_new(Source), in.res, in.prec, [Time, aif, in.nb], Win=win[k])
    		Source = TRANSPOSE(MOCO_2D_DCE_1CM->deformed(), [1,2,0])
    		OBJ_DESTROY, MOCO_2D_DCE_1CM
		endif
...
pro PMI__Button__Control__MOCO_2D_DCE_1CM, id, v
...
function PMI__Button__MOCO_2D_DCE_1CM, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'PK motion correction'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__MOCO_2D_DCE_1CM' $
    ,	pro_set_value = 'PMI__Button__Control__MOCO_2D_DCE_1CM' $
    ,  	separator = separator )
