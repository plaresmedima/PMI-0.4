...
FUNCTION PMI__Button__Conc__FitModToftsLin, Signal, S0, Model
...
FUNCTION PMI__Button__Input__FitModToftsLin, top, Series, aif, in, vof=vof

    PMI__Info, top, Stdy=Stdy
    DynSeries = Stdy->Names(0,DefDim=3,ind=ind,sel=sel)
	in = {ser:sel, aif:stdy->sel(1), vof:0, sig:0, nb:1, hct:0.45}
...
;		ptr_new({Type:'DROPLIST',Tag:'vof', Label:'Venous Region', Value:['<none>',Stdy->names(1)], Select:in.vof}), $
		ptr_new({Type:'DROPLIST',Tag:'sig', Label:'Signal model:', Value:['Linear (a.u.)','Linear (%)', 'DSC-MRI'], Select:in.sig}), $
...
pro PMI__Button__Event__FitModToftsLin, ev

	PMI__Info, ev.top, Status=Status, Stdy=Stdy
	PMI__Message, status, 'Preparing calculation..'

    IF NOT PMI__Button__Input__FitModToftsLin(ev.top,series,aif,in,vof=vof) THEN RETURN
	...
	Aif = PMI__Button__Conc__FitModToftsLin(Aif,total(AIF[0:in.nb-1])/in.nb,in.sig)/(1-in.hct)
	if n_elements(vof) ne 0 then begin
    	Vof = PMI__Button__Conc__FitModToftsLin(Vof,total(VOF[0:in.nb-1])/in.nb,in.sig)/(1-in.hct)
	...
	for j=0L,d[2]-1 do begin
    	...
    	P = PMI__Button__Conc__FitModToftsLin(P,P0,in.sig)
...
pro PMI__Button__Control__FitModToftsLin, id, v
...
function PMI__Button__FitModToftsLin, parent,value=value,separator=separator

    if n_elements(value) eq 0 then value = 'Modified Tofts (Pixel)'

    id = widget_button(parent $
    ,   value = value  $
    ,  	event_pro = 'PMI__Button__Event__FitModToftsLin' $
    ,	pro_set_value = 'PMI__Button__Control__FitModToftsLin' $
    ,  	separator = separator )
