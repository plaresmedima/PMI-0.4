...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display::Fit
...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display::Plot
	...
	CASE OnDisplay OF
		...
		'VOF':begin
			Self -> GET, Time=Time, VofCurve=Y, VofName=VofName
			Self -> SET, /Erase
			if n_elements(Y) eq 0 then return
 			plot, time, Y, position=[0.1,0.2,0.5,0.9]  $
			, 	/xstyle, /ystyle $
			, 	background=255, color=0 $
			, 	xtitle = 'Time (sec)', ytitle=self.Units $
		...
		'FIT':BEGIN
			Self -> GET, RoiCurve=Curve, Time=Time, Fit=Fit, Model=Model, RoiName=RoiName, AifName=AifName, VofName=VofName
			Self -> SET, /Erase
...
FUNCTION PMI__Button__FitSingleInletRoiPVcorr__Display::Event, ev
	...
	If cnt eq 1 then begin
		...
		if index eq 3 then ptr_free, Self.Curve[1]
...
FUNCTION PMI__Button__FitSingleInletRoiPVcorr__Display__Event, ev
...
FUNCTION PMI__Button__FitSingleInletRoiPVcorr__Display::GetCurve, List
	...
	if select LT 0 then return, 0
	Region = Stdy->Obj(1,select)
...
FUNCTION PMI__Button__FitSingleInletRoiPVcorr__Display::GetName, List
...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display::GET $
, 	CursorPos = CursorPos $
...
, 	OnDisplay=OnDisplay
	...
	if arg_present(AifCurve) then $
		if ptr_valid(Self.Curve[1]) then AifCurve = *Self.Curve[1] $
			...
			If n_elements(Vof) ne 0 then begin ;correct partial volume
				self->get, time=t
			;	IRF = DeconvolveCurve(t,vof, AifCurve, dt=dt, pc='GCV', wm=1L, m0=0.001, m1=1.0, nm=100L, Quad='O2')
				IRF = DeconvolveCurve(t,vof, AifCurve, dt=dt, REGPAR=0.1, Quad='O2')
				PvCorr = dt*total(IRF)
			;	PvCorr = total(vof)/total(AifCurve)
			;	PvCorr = max(vof)/max(AifCurve)
				AifCurve = AifCurve*PvCorr
			endif
			Self.Curve[1] = ptr_new(AifCurve)
		endelse

	if arg_present(VofCurve) then $
		if ptr_valid(Self.Curve[3]) then VofCurve = *Self.Curve[3] $
		else begin
			select = widget_info(widget_info(self.id,find_by_uname='VOF'),/droplist_select)
			if select NE 0 then begin
				VofCurve = Self->GetCurve('VOF')/(1-self.Hematocrit)
				Self.Curve[3] = ptr_new(VofCurve)
				ptr_free, Self.Curve[1]
			endif
		endelse
...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display::SET, $
	...
	or n_elements(ysize) ne 0 then begin
		...
		xs = floor((xsize - 500)/4E)
		if xs lt 50 then xs = 50
		List = ['ROI','AIF','VOF','FIT']
...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display::Cleanup
	widget_control, self.id, /destroy
	ptr_free, Self.Curve, self.Parameters
	loadct, 0
END

FUNCTION PMI__Button__FitSingleInletRoiPVcorr__Display::Init, parent, CursorPos, xsize=xsize, ysize=ysize
	...
	self.id = widget_base(parent,/column,map=0,event_func='PMI__Button__FitSingleInletRoiPVcorr__Display__Event')
...
PRO PMI__Button__FitSingleInletRoiPVcorr__Display__Define

	Struct = {PMI__Button__FitSingleInletRoiPVcorr__Display 	$
	,	id: 0L 	$
	,	DrawId: 0L $
	,	CursorPos:lonarr(4)	$
...
FUNCTION PMI__Button__Input__FitSingleInletRoiPVcorr, top, series, in, units
...
pro PMI__Button__Event__FitSingleInletRoiPVcorr, ev

	IF NOT PMI__Button__Input__FitSingleInletRoiPVcorr(ev.top, series, in, units) THEN return

	PMI__Control, ev.top, Viewer = 'PMI__Button__FitSingleInletRoiPVcorr__Display', Display=Display
...
pro PMI__Button__Control__FitSingleInletRoiPVcorr, id, v
...
function PMI__Button__FitSingleInletRoiPVcorr, parent,value=value, separator=separator
	...
	,	value = value	$
	,	event_pro = 'PMI__Button__Event__FitSingleInletRoiPVcorr'	$
	,	pro_set_value = 'PMI__Button__Control__FitSingleInletRoiPVcorr' $
	, 	separator = separator )
	return, id
end
